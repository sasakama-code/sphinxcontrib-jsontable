...
def demo_quick_conversion():
    ...
    try:
        ...
        sales_result = convert_excel_to_rag(
            excel_file=sales_file, rag_purpose="sales-analysis", auto_sphinx_docs=True
        )
...
def demo_advanced_features():
    ...
    try:
        converter.set_rag_system(
            "openai",
            {"model": "text-embedding-3-small", "api_key": "your-api-key-here"},
        )
        print("   ✅ OpenAI設定完了")
    except Exception as e:
    ...
    try:
        converter.set_rag_system(
            "langchain", {"vectorstore": "chroma", "llm": "gpt-3.5-turbo"}
        )
        print("   ✅ LangChain設定完了")
    except Exception as e:
    ...
    try:
        converter.set_rag_system(
            "custom", {"endpoint": "https://your-rag-api.com", "api_key": "custom-key"}
        )
        print("   ✅ カスタムRAG設定完了")
    except Exception as e:
...
def demo_business_scenarios():
    ...
    for scenario in scenarios:
        print(f"\n📋 {scenario['name']}")
        print(
            f"   設定: {json.dumps(scenario['config'], ensure_ascii=False, indent=6)}"
        )
        print("   ✅ 設定適用可能")
