...
def demo_federation_setup():
    ...
    departments = {
        "sales": {
            ...
            "config": {"industry_focus": "retail", "access_level": "internal"},
        },
        "finance": {
            "name": "財務部",
            "excel_sources": [
            ...
            "config": {"industry_focus": "financial", "access_level": "confidential"},
        },
        "production": {
            "name": "製造部",
            "excel_sources": [
                {
                    "file": enterprise_files["production"],
                    "purpose": "production-management",
                }
            ],
            "config": {"industry_focus": "manufacturing", "access_level": "internal"},
        },
    }
    ...
    federation = create_enterprise_federation(
        departments=departments, federation_name="demo-enterprise"
    )

    print("✅ 企業統合システム作成完了!")
    print(f"   - 統合部署数: {len(departments)}")
    print(
        f"   - 総Excelファイル数: {sum(len(dept['excel_sources']) for dept in departments.values())}"
    )
...
def demo_cross_department_analysis(federation: ExcelRAGFederation):
    ...
    print(
        f"   - 統合分析可能フィールド数: {len(result.get('unified_schema', {}).get('common_fields', {}))}"
    )
...
def demo_executive_reporting(federation: ExcelRAGFederation):
    ...
    try:
        report = federation.generate_executive_report(
            target_personas=executive_personas, report_format="comprehensive"
        )
        ...
        print("\n👔 CEO向けエグゼクティブサマリー:")
        print(
            f"   - 重要指標: {ceo_report.get('dashboard_metrics', {}).get('primary_kpi', 'N/A')}"
        )
        print(
            f"   - トレンド: {ceo_report.get('dashboard_metrics', {}).get('trend_indicator', 'N/A')}"
        )
        print(
            f"   - リスクスコア: {ceo_report.get('dashboard_metrics', {}).get('risk_score', 'N/A')}"
        )
...
def demo_real_time_monitoring(
    federation: ExcelRAGFederation, enterprise_files: dict[str, str]
):
    """リアルタイム監視デモ."""
    print("\n⚡ リアルタイムExcel監視システム")
    print("=" * 45)
    ...
    # 各部署ファイルを監視対象に追加
    departments = {"sales": "営業部", "finance": "財務部", "production": "製造部"}
        ...
        if dept_id in enterprise_files:
            try:
                monitor.watch_file(
                    ...
                    on_change=lambda d=dept_name: print(
                        f"📄 {d}のExcelファイルが更新されました!"
                    ),
                )
