...
class TestPhase1And2Integration:
    ...
    def test_complete_pipeline_japanese_employees(self):
        ...
        # カテゴリカルファセット（部署）
        dept_facets = [f for f in generated_facets.categorical_facets if f.field_name == "部署"]
        ...
        # 数値ファセット（給与）
        salary_facets = [f for f in generated_facets.numerical_facets if f.field_name == "給与"]
    ...
    def test_business_data_pipeline(self):
        ...
        # 価格帯ファセットの確認
        price_facets = [f for f in generated_facets.numerical_facets if f.field_name == "価格"]
    ...
    def test_multilingual_support(self):
        ...
        semantic_chunks = self.semantic_chunker.process(multilingual_data, basic_metadata)
        advanced_metadata = self.advanced_generator.generate_advanced_metadata(
        ...
        # 多言語対応の確認
        english_entities = [
            e for e in advanced_metadata.entity_classification.persons
    ...
    def test_large_dataset_performance(self):
        ...
        summary_chunks = [c for c in semantic_chunks if c.chunk_type == "statistical_summary"]
        assert len(summary_chunks) > 0

    def test_export_format_completeness(self):
        """全出力形式の完全性テスト"""
        test_data = [
            {"製品": "テスト製品", "価格": 1000, "カテゴリ": "テスト"}
        ]
