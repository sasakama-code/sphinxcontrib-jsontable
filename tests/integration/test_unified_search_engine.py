...
class TestUnifiedSearchEngineInitialization:
    ...
    def test_initialization_with_custom_config(
        ...
        engine = UnifiedSearchEngine(
            vector_config=vector_config_disabled, search_config=search_config_default
        )
...
class TestUnifiedSearchEngineSearch:
    ...
    async def test_vector_search_disabled_mode(self, indexed_engine):
        ...
        query = SearchQuery(
            query_text="営業戦略", query_type="vector", max_results=3, min_score=0.0
        )
    ...
    async def test_faceted_search(self, indexed_engine):
        ...
        query = SearchQuery(
            query_text="",
            query_type="faceted",
            filters={"categorical": {"business_category": ["financial"]}},
            max_results=5,
        )
...
class TestUnifiedSearchEnginePerformance:
    ...
    async def test_search_caching(self, indexed_engine):
        ...
        query = SearchQuery(query_text="売上分析", query_type="semantic", max_results=3)
...
class TestUnifiedSearchEngineErrorHandling:
    ...
    async def test_invalid_query_type(self, indexed_engine):
        ...
        query = SearchQuery(query_text="test", query_type="invalid_type", max_results=1)
