...
from sphinxcontrib.jsontable.rag.semantic_chunker import SemanticChunker, SemanticChunk
from sphinxcontrib.jsontable.rag.advanced_metadata import AdvancedMetadataGenerator, AdvancedMetadata
...
class TestPhase1Integration:
    ...
    def test_phase1_basic_workflow(self):
        ...
        metadata = self.metadata_extractor.extract_metadata(sample_data)
    ...
    def test_phase1_japanese_text_processing(self):
        ...
        metadata = self.metadata_extractor.extract_metadata(japanese_data)
    ...
    def test_phase1_error_handling(self):
        ...
        # Test empty data
        empty_metadata = self.metadata_extractor.extract_metadata([])
        assert empty_metadata.record_count == 0
        
        # Test malformed data
        try:
            self.metadata_extractor.extract_metadata(None)
        except Exception:
...
class TestPhase2Integration:
    ...
    def test_phase2_advanced_workflow(self):
        ...
        business_data = [
            {"company": "株式会社ABC", "revenue": 5000000, "industry": "製造業", "location": "東京都"},
            {"company": "XYZ商事", "revenue": 3000000, "industry": "商業", "location": "大阪府"},
            {"company": "テクノロジー株式会社", "revenue": 8000000, "industry": "IT", "location": "神奈川県"}
        ]
        
        # Step 1: Basic metadata extraction
        basic_metadata = self.metadata_extractor.extract_metadata(business_data)
        assert basic_metadata.record_count == 3
        
        # Step 2: Advanced metadata generation
        advanced_metadata = self.advanced_generator.generate_advanced_metadata(business_data, basic_metadata)
    ...
    def test_phase2_japanese_entity_recognition(self):
        ...
        entity_data = [
            {"person": "田中太郎", "organization": "株式会社サンプル", "location": "東京都渋谷区"},
            {"person": "佐藤花子", "organization": "テスト商事", "location": "大阪市中央区"}
        ]
        
        basic_metadata = self.metadata_extractor.extract_metadata(entity_data)
        advanced_metadata = self.advanced_generator.generate_advanced_metadata(entity_data, basic_metadata)
    ...
    def test_phase2_export_formats(self):
        ...
        advanced_metadata = self.advanced_generator.generate_advanced_metadata(sample_data, basic_metadata)
...
class TestPhase3Integration:
    ...
    def test_phase3_vector_processing(self):
        ...
        try:
            ...
            vectors = self.vector_processor.generate_vectors([item["content"] for item in japanese_text_data])
    ...
    def test_phase3_search_index_creation(self):
        ...
        index_data = [
            ...
            {"id": 3, "title": "プロジェクトC", "description": "革新的なプロジェクトCの概要"}
        ]
...
class TestFullIntegrationWorkflow:
    ...
    def test_complete_rag_workflow(self):
        ...
        basic_metadata = self.metadata_extractor.extract_metadata(comprehensive_data)
        assert basic_metadata.record_count == 3
        ...
        advanced_metadata = self.advanced_generator.generate_advanced_metadata(comprehensive_data, basic_metadata)
        assert isinstance(advanced_metadata, AdvancedMetadata)
    ...
    def test_enhanced_directive_integration(self):
        ...
        directive = EnhancedJsonTableDirective(
            ...
            content=['[{"product": "ノートPC", "price": 120000, "category": "電子機器"}]'],
            lineno=1,
            content_offset=0,
            block_text="",
            state=mock_state,
    ...
    def test_performance_with_realistic_data(self):
        ...
        for i in range(50):  # Realistic business dataset size
            realistic_data.append({
                ...
                "join_year": 2015 + (i % 10),
                "location": "東京都" if i % 3 == 0 else "大阪府" if i % 3 == 1 else "神奈川県"
            })
        
        # Process through all phases
        basic_metadata = self.metadata_extractor.extract_metadata(realistic_data)
        assert basic_metadata.record_count == 50
        
        advanced_metadata = self.advanced_generator.generate_advanced_metadata(realistic_data, basic_metadata)
        assert advanced_metadata is not None
