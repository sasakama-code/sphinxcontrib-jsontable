...
class TestExcelRAGConverter:
    ...
    @patch('sphinxcontrib.jsontable.excel.converter.ExcelRAGConverter._validate_excel_file')
    @patch('sphinxcontrib.jsontable.excel.converter.ExcelRAGConverter._generate_rag_metadata')
    @patch('sphinxcontrib.jsontable.excel.converter.ExcelRAGConverter._generate_output_files')
    @patch('sphinxcontrib.jsontable.excel.converter.ExcelRAGConverter._generate_sphinx_documentation')
    @patch('sphinxcontrib.jsontable.excel.converter.ExcelRAGConverter._calculate_overall_quality')
    def test_convert_excel_to_rag_success(
        self, 
        mock_quality, mock_sphinx, mock_output, mock_metadata, mock_validate,
        converter, mock_excel_file, sample_conversion_result
    ):
        """Test successful Excel-to-RAG conversion."""
        # Setup mocks
        mock_validate.return_value = None
        converter.excel_converter.convert_with_intelligence = Mock(return_value=sample_conversion_result)
    ...
    @patch('sphinxcontrib.jsontable.excel.converter.ExcelRAGConverter._validate_excel_file')
    def test_convert_excel_to_rag_validation_error(self, mock_validate, converter, mock_excel_file):
    ...
    def test_generate_rag_metadata(self, converter, sample_conversion_result):
        ...
        converter.rag_extractor.extract_metadata = Mock(return_value={'basic': 'metadata'})
        converter.advanced_metadata.generate_advanced_metadata = Mock(return_value={'advanced': 'metadata'})
    ...
    def test_generate_output_files(self, converter, sample_conversion_result):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                result = converter._generate_output_files(sample_conversion_result, metadata, config)
                ...
                with open("test-analysis_data.json", 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    assert data == sample_conversion_result['json_data']
                
                with open("test-analysis_metadata.json", 'r', encoding='utf-8') as f:
                    meta = json.load(f)
    ...
    def test_calculate_overall_quality(self, converter, sample_conversion_result):
        ...
        quality_score = converter._calculate_overall_quality(sample_conversion_result, metadata)
...
class TestAdvancedExcelConverter:
    ...
    def test_extract_business_entities(self, converter):
        ...
        result = converter._extract_business_entities(sample_data, "general", "japanese")
...
class TestAutoSphinxIntegration:
    ...
    def test_generate_example_queries_sales(self, integration, sample_conversion_result):
        """Test example query generation for sales analysis."""
        config = {'rag_purpose': 'sales-analysis'}
        queries = integration._generate_example_queries(sample_conversion_result, config)
        
        assert isinstance(queries, str)
        assert '営業担当者' in queries
        assert '売上' in queries
    
    def test_generate_example_queries_inventory(self, integration, sample_conversion_result):
    ...
    def test_generate_advanced_queries(self, integration, sample_conversion_result):
        ...
        queries = integration._generate_advanced_queries(sample_conversion_result, config)
    ...
    def test_create_sub_documents_small_dataset(self, integration, sample_conversion_result):
    ...
    def test_create_index_document(self, integration, sample_conversion_result):
        ...
        index_doc = integration._create_index_document(sample_conversion_result, main_doc, sub_docs)
    ...
    def test_update_sphinx_config(self, integration, sample_conversion_result):
        ...
        sphinx_config = integration._update_sphinx_config(sample_conversion_result, config)
...
class TestFormatHandlers:
    ...
    def test_excel_format_handler_initialization(self):
        ...
        expected_handlers = [
            'pivot_table', 'financial_statement', 'multi_header',
            'cross_tab', 'time_series', 'standard_table'
        ]
...
class TestConvenienceFunctions:
    ...
    def test_convert_excel_to_rag_function(self, mock_converter_class):
        ...
        result = convert_excel_to_rag("test.xlsx", "test-purpose", test_arg="test_value")
    ...
    def test_query_excel_data_function(self, mock_converter_class):
        ...
        mock_converter_class.assert_called_once()
        mock_converter.query_excel_data.assert_called_once_with("test.xlsx", "Test question")
