...
from sphinxcontrib.jsontable.errors.error_handler_core import (
    ErrorHandlerCore,
    IErrorHandler,
)
...
class TestErrorHandlerCoreLogging:
    ...
    def test_log_structured_error_data(self):
        ...
        with patch.object(self.handler.logger, "error") as mock_logger_error:
            self.handler._log_structured_error(
                error,
                context,
                {
...
class TestErrorHandlerCorePerformance:
    ...
    def test_error_handling_performance(self):
        """エラーハンドリングの性能テスト"""
        errors = [FileAccessError(f"Error {i}") for i in range(100)]
...
class TestErrorHandlerCoreIntegration:
    ...
    def setup_method(self):
        self.handler = ErrorHandlerCore(enable_recovery=True, max_retry_attempts=3)
        ...
        with patch.object(self.handler, "_execute_retry") as mock_retry:
            ...
            mock_retry.side_effect = [
                ...
                HandlingResult(
                    success=True,
                    recovery_attempted=True,
                    recovered_data="Success on retry",
                ),
            ]
