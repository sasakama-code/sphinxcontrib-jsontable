...
class TestConcurrentProcessingOptimization:
    ...
    def test_concurrent_excel_processing_optimization(self, processor, test_files):
        ...
        assert (
            perf_metrics.concurrent_speedup_factor >= CONCURRENT_SPEEDUP_TARGET
        )  # 4倍以上高速化
        assert (
            perf_metrics.thread_utilization_efficiency >= THREAD_EFFICIENCY_TARGET
        )  # 85%以上効率
        ...
        assert (
            resource_metrics.contention_rate <= RESOURCE_CONTENTION_MAX
        )  # 10%以下競合率
    ...
    def test_async_processing_optimization(self, processor, test_files):
        ...
        assert (
            async_metrics.async_throughput_per_second >= ASYNC_THROUGHPUT_TARGET
        )  # 500件/秒以上
        ...
        print(
            f"Async throughput: {async_metrics.async_throughput_per_second:.0f} items/sec"
        )
    ...
    def test_resource_contention_minimization(self, processor, test_files):
        ...
        assert (
            contention_metrics.contention_rate <= RESOURCE_CONTENTION_MAX
        )  # 10%以下競合率
        assert (
            contention_metrics.resource_utilization_efficiency >= 0.90
        )  # 90%以上利用効率
        ...
        print(
            f"Resource efficiency: {contention_metrics.resource_utilization_efficiency:.1%}"
        )
    ...
    def test_scalability_performance_measurement(self, processor, test_files):
        ...
        print(
            f"Sustained throughput: {scalability_metrics.sustained_throughput:.0f} items/sec"
        )
    ...
    def test_concurrent_processing_benchmark(self, processor, test_files):
        ...
        assert (
            performance_comparison.concurrent_vs_sequential_speedup >= 5.0
        )  # 5倍以上高速化
        assert (
            performance_comparison.async_vs_sequential_speedup >= 4.0
        )  # 4倍以上高速化
        assert (
            performance_comparison.cpu_utilization_improvement >= 0.50
        )  # 50%以上CPU改善

        # メモリ効率性検証
        memory_comparison = result.memory_efficiency_comparison
        assert (
            memory_comparison.concurrent_memory_efficiency >= 0.85
        )  # 85%以上メモリ効率
        ...
        print(
            f"Concurrent speedup: {performance_comparison.concurrent_vs_sequential_speedup:.1f}x"
        )
        print(
            f"Async speedup: {performance_comparison.async_vs_sequential_speedup:.1f}x"
        )
        print(
            f"CPU improvement: {performance_comparison.cpu_utilization_improvement:.1%}"
        )
