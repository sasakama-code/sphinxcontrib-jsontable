...
class TestMemoryUsageBenchmark:
    ...
    def create_benchmark_test_file(
        self, size_mb: int = 50, filename: str = "benchmark_test.xlsx"
    ) -> Path:
        ...
        for chunk_start in range(0, estimated_rows, chunk_size):
            ...
            chunk_data = {
                ...
                "Category": [
                    f"Category_{i % 50}" for i in range(chunk_start, chunk_end)
                ],
                "Description": [
                    f"Desc {i}" for i in range(chunk_start, chunk_end)
                ],  # 短縮
            }
    ...
    def test_memory_usage_benchmark_comprehensive(self):
        ...
        baseline_result = benchmarker.benchmark_traditional_processing(
            file_path=benchmark_file, chunk_size=1000
        )
        ...
        print(
            f"Memory improvement: Peak {baseline_result.peak_memory_mb:.1f}MB -> {optimized_result.peak_memory_mb:.1f}MB"
        )
    ...
    def test_component_memory_profiling(self):
        ...
        """
        # テストファイル作成
        test_file = self.create_benchmark_test_file(
            size_mb=40, filename="component_profile.xlsx"
        )
    ...
    def test_baseline_comparison_analysis(self):
        ...
        """
        # 複数サイズのテストファイル作成
        small_file = self.create_benchmark_test_file(
            size_mb=20, filename="small_test.xlsx"
        )
        medium_file = self.create_benchmark_test_file(
            size_mb=50, filename="medium_test.xlsx"
        )
        large_file = self.create_benchmark_test_file(
            size_mb=80, filename="large_test.xlsx"
        )
        ...
        for size_label, file_path in test_files.items():
            baselines[size_label] = comparator.benchmark_traditional_processing(
                file_path
            )
            optimized_results[size_label] = comparator.benchmark_optimized_processing(
                file_path
            )

        # ベースライン比較分析
        comparison_result = comparator.compare_with_baselines(
            baselines=baselines, optimized_results=optimized_results
        )
    ...
    def test_memory_efficiency_metrics(self):
        ...
        """
        # ベンチマークファイル作成
        efficiency_file = self.create_benchmark_test_file(
            size_mb=70, filename="efficiency_test.xlsx"
        )
        ...
        efficiency_result = efficiency_benchmarker.benchmark_memory_efficiency(
            file_path=efficiency_file, duration_seconds=30
        )
    ...
    def test_performance_visualization_data(self):
        ...
        """
        # 可視化用ファイル作成
        viz_file = self.create_benchmark_test_file(
            size_mb=45, filename="visualization_test.xlsx"
        )
    ...
    def test_benchmark_reporting(self):
        ...
        """
        # 報告書用ファイル作成
        report_file = self.create_benchmark_test_file(
            size_mb=55, filename="report_test.xlsx"
        )
    ...
    def test_continuous_benchmarking(self):
        ...
        """
        # 継続監視用ファイル作成
        continuous_file = self.create_benchmark_test_file(
            size_mb=35, filename="continuous_test.xlsx"
        )
        ...
        monitoring_session = continuous_benchmarker.start_continuous_benchmarking(
            file_path=continuous_file, baseline_iterations=3, monitoring_duration=20.0
        )
        ...
        # トレンド分析確認
        trend_analysis = continuous_benchmarker.get_trend_analysis(
        ...
        # 回帰検出確認
        regression_status = continuous_benchmarker.check_regression_status(
            monitoring_session.session_id
        )
        ...
        # 監視停止
        continuous_benchmarker.stop_continuous_benchmarking(
            monitoring_session.session_id
        )
