...
class TestChunkProcessingOptimization:
    ...
    def create_large_test_excel_file(
        self, rows: int = 10000, filename: str = "large_test.xlsx"
    ) -> Path:
        """大容量テスト用Excelファイル作成."""
        ...
        data = {
            ...
            "Description": [
                f"This is a very long description for row {i} with lots of text data to increase memory usage"
                for i in range(rows)
            ],
            "Timestamp": [
    ...
    def test_chunk_processing_large_file(self):
        ...
        traditional_reader = StreamingExcelReader(
            chunk_size=1000, enable_monitoring=True
        )
        ...
        assert sum(len(chunk.data) for chunk in traditional_chunks) == sum(
            len(chunk.data) for chunk in optimized_chunks
        )
    ...
    def test_adaptive_chunk_sizing(self):
        ...
        medium_file = self.create_large_test_excel_file(
            rows=5000, filename="medium.xlsx"
        )
        large_file = self.create_large_test_excel_file(
            rows=15000, filename="large.xlsx"
        )
        ...
        assert (
            small_metrics["effective_chunk_size"]
            <= medium_metrics["effective_chunk_size"]
        )
        assert (
            medium_metrics["effective_chunk_size"]
            <= large_metrics["effective_chunk_size"]
        )
