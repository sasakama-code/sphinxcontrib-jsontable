...
class TestFileLevelCache:
    ...
    def create_test_excel_file(
        self, filename: str = "test_cache.xlsx", rows: int = 1000
    ) -> Path:
        """テスト用Excelファイル作成
    ...
    def test_lru_cache_eviction_policy(self):
        ...
        """
        # 小さなキャッシュサイズで制限テスト
        cache_config = CacheConfiguration(max_cache_size=3, lru_eviction_enabled=True)
        file_cache = FileLevelCache(cache_config)
    ...
    def test_cache_performance_improvement(self):
        ...
        cache_config = CacheConfiguration(
            max_cache_size=5, max_memory_mb=50, enable_performance_tracking=True
        )
        file_cache = FileLevelCache(cache_config)
    ...
    def test_compressed_cache_storage(self):
        ...
        compressed_config = CacheConfiguration(
            enable_compression=True, compression_level=6, compression_algorithm="gzip"
        )
        ...
        large_data = {
            ...
            "metadata": {
                ...
                "processing_info": {
                    "step_" + str(i): f"data_{i}" * 50 for i in range(100)
                },
            },
        }

        cache_key = compressed_cache.generate_cache_key(
            test_file, {"compression_test": True}
        )
    ...
    def test_cache_memory_efficiency(self):
        ...
        for i, test_file in enumerate(test_files):
            ...
            try:
                ...
                # メモリ制限遵守確認
                assert (
                    current_memory <= memory_config.max_memory_mb * 1.1
                )  # 10%マージン
