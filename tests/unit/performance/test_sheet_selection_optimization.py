...
from sphinxcontrib.jsontable.performance.sheet_selection_optimizer import (
    ...
    SheetSelectionMetrics,
    SheetSelectionResult,
    MemoryOptimizationMetrics,
    MemoryOptimizationResult,
    IOOptimizationMetrics,
    IOOptimizationResult,
    MultiSheetProcessingMetrics,
    MultiSheetProcessingResult,
    LazySheetIntegrationMetrics,
...
def sheet_selection_components():
    """シート選択最適化コンポーネント"""
    return {
        'sheet_selection_optimizer': SheetSelectionOptimizer()
    }
...
def multi_sheet_test_file(tmp_path):
    ...
    with pd.ExcelWriter(tmp_path / "multi_sheet_test.xlsx", engine='openpyxl') as writer:
        # Sheet1: 大容量データ（3000行）
        data1 = {
            f'col_{i}': list(range(100 + i, 3100 + i)) 
            for i in range(8)
        }
        ...
        # Sheet2: 中容量データ（1500行）
        data2 = {
            f'data_{i}': list(range(200 + i, 1700 + i)) 
            for i in range(6)
        }
        ...
        # Sheet3: 小容量データ（500行）
        data3 = {
            f'info_{i}': list(range(300 + i, 800 + i)) 
            for i in range(4)
        }
...
class TestSheetSelectionOptimization:
    ...
    def test_target_sheet_only_loading(self, sheet_selection_components, multi_sheet_test_file):
        """対象シートのみ読み込み機能確認
        ...
        - 読み込み時間短縮
        """
        result = sheet_selection_components['sheet_selection_optimizer'].implement_target_sheet_only_loading(
        ...
        assert selection_metrics.loading_time_reduction >= 0.50  # 50%以上読み込み時間削減
        assert selection_metrics.sheet_selection_response_time_ms <= RESPONSE_TIME_TARGET

    def test_memory_optimization_through_sheet_selection(self, sheet_selection_components, multi_sheet_test_file):
        """シート選択によるメモリ最適化確認
        ...
        - 大容量ファイル対応
        """
        result = sheet_selection_components['sheet_selection_optimizer'].optimize_memory_through_sheet_selection(
            multi_sheet_test_file,
            {
        ...
        assert memory_metrics.memory_optimization_effectiveness >= MEMORY_OPTIMIZATION_TARGET
        ...
        assert memory_metrics.large_file_memory_efficiency >= 0.85  # 85%以上大容量ファイル効率

    def test_io_optimization_through_sheet_selection(self, sheet_selection_components, multi_sheet_test_file):
        """シート選択によるI/O最適化確認
        ...
        - 並列I/O対応
        """
        result = sheet_selection_components['sheet_selection_optimizer'].optimize_io_through_sheet_selection(
            multi_sheet_test_file,
            {
        ...
        assert io_metrics.network_transfer_reduction >= 0.70      # 70%以上ネットワーク転送削減
        assert io_metrics.io_response_time_ms <= 120             # 120ms以下I/O応答時間

    def test_multi_sheet_processing_optimization(self, sheet_selection_components, multi_sheet_test_file):
        """複数シート処理最適化確認
        ...
        - リソース効率分散
        """
        result = sheet_selection_components['sheet_selection_optimizer'].optimize_multi_sheet_processing(
            multi_sheet_test_file,
            {
        ...
        assert multi_metrics.multi_sheet_processing_effectiveness >= MULTI_SHEET_PROCESSING_TARGET
    ...
    def test_lazy_loading_sheet_integration(self, sheet_selection_components, multi_sheet_test_file):
        """遅延読み込み・シート選択統合確認
        ...
        - 統合I/O効率化
        """
        result = sheet_selection_components['sheet_selection_optimizer'].integrate_with_lazy_loading(
            multi_sheet_test_file,
            {
        ...
        assert integration_metrics.lazy_sheet_integration_effectiveness >= LAZY_INTEGRATION_TARGET
        assert integration_metrics.synergy_effect_score >= 0.80             # 80%以上相乗効果
        assert integration_metrics.unified_optimization_score >= 0.85       # 85%以上統合最適化
        assert integration_metrics.integration_performance_improvement >= 0.75  # 75%以上統合性能向上

    def test_sheet_selection_integration_verification(self, sheet_selection_components, multi_sheet_test_file):
        """シート選択最適化統合確認
        ...
        - スケーラビリティ確保
        """
        result = sheet_selection_components['sheet_selection_optimizer'].verify_sheet_selection_integration(
            multi_sheet_test_file,
            {
...
class TestSheetSelectionOptimizationEdgeCases:
    ...
    def test_single_sheet_file_optimization(self, sheet_selection_components, tmp_path):
        ...
        result = sheet_selection_components['sheet_selection_optimizer'].implement_target_sheet_only_loading(
            single_file,
            {"enable_sheet_selection": True, "target_sheets": ["Sheet1"]}
        )
        
        # 単一シートでもエラーなく処理される
        assert result.sheet_selection_success == True

    def test_large_multi_sheet_file_optimization(self, sheet_selection_components, tmp_path):
        """大容量複数シートファイル最適化処理"""
        import pandas as pd
        
        # 大容量複数シートファイル作成
        with pd.ExcelWriter(tmp_path / "large_multi_sheet.xlsx", engine='openpyxl') as writer:
        ...
        assert result.memory_optimization_success == True
        assert result.memory_optimization_metrics.memory_usage_reduction >= 0.75  # 大容量では75%以上削減期待

    def test_corrupted_sheet_selection_error_handling(self, sheet_selection_components, tmp_path):
