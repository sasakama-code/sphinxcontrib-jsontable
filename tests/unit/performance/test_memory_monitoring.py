...
class TestMemoryMonitor:
    ...
    def create_test_excel_file(
        self, rows: int = 5000, filename: str = "memory_test.xlsx"
    ) -> Path:
        ...
        data = {
            ...
            "LargeText": [
                f"Large text data with lots of content for row {i} " * 10
                for i in range(rows)
            ],
            "NumericData": [i * 123.456789 for i in range(rows)],
            "CategoryData": [f"Category{i % 100}_WithLongName" for i in range(rows)],
            "Description": [
                f"Very long description text for testing memory usage in row {i} " * 5
                for i in range(rows)
            ],
        }
    ...
    def test_memory_threshold_alerts(self):
        ...
        monitor = MemoryMonitor(
            monitoring_interval=0.05, alert_config=alert_config, enable_alerts=True
        )
    ...
    def test_memory_optimization_triggers(self):
        ...
        def optimization_callback(
            optimization_type: str, before_memory: int, after_memory: int
        ):
    ...
    def test_memory_monitoring_thread_safety(self):
        ...
        """
        monitor = MemoryMonitor(
            monitoring_interval=0.05, enable_history=True, thread_safe=True
        )
