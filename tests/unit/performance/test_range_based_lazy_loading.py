...
def range_lazy_components():
    """データ範囲遅延読み込みコンポーネント"""
    return {
        'range_lazy_loader': RangeLazyLoader()
    }
...
def multi_sheet_range_file(tmp_path):
    ...
    with pd.ExcelWriter(tmp_path / "multi_sheet_range_test.xlsx", engine='openpyxl') as writer:
        # Sheet1: 大容量データ（8000行×15列）
        data1 = {
            f'data_{i}': list(range(i * 500, i * 500 + 8000)) 
            for i in range(15)
        }
        ...
        data2 = {
            f'info_{i}': list(range(i * 200, i * 200 + 3000)) 
            for i in range(10)
        }
        ...
        data3 = {
            f'meta_{i}': list(range(i * 100, i * 100 + 1000)) 
            for i in range(5)
        }
...
class TestRangeBasedLazyLoading:
    ...
    def test_range_specification_loading(self, range_lazy_components, structured_excel_file):
        """範囲指定読み込み機能確認
        ...
        - メモリ使用量大幅削減
        """
        result = range_lazy_components['range_lazy_loader'].implement_range_specification_loading(
    ...
    def test_partial_data_lazy_loading(self, range_lazy_components, structured_excel_file):
        """部分データ遅延読み込み確認
        ...
        - 高効率部分アクセス
        """
        result = range_lazy_components['range_lazy_loader'].implement_partial_data_lazy_loading(
            structured_excel_file,
            {
        ...
        assert partial_metrics.partial_loading_efficiency >= 0.85       # 85%以上部分読み込み効率
    ...
    def test_memory_efficiency_through_range_loading(self, range_lazy_components, structured_excel_file):
        """範囲読み込みによるメモリ効率確認
        ...
        - メモリリーク防止
        """
        result = range_lazy_components['range_lazy_loader'].optimize_memory_efficiency_through_range_loading(
            structured_excel_file,
            {
    ...
    def test_lazy_loading_foundation_integration(self, range_lazy_components, structured_excel_file):
        """遅延読み込み基盤統合確認
        ...
        - 統合パフォーマンス向上
        """
        result = range_lazy_components['range_lazy_loader'].integrate_with_lazy_loading_foundation(
            structured_excel_file,
            {
        ...
        assert integration_metrics.lazy_integration_effectiveness >= LAZY_INTEGRATION_TARGET
    ...
    def test_cache_integration_optimization(self, range_lazy_components, structured_excel_file):
        """キャッシュ統合最適化確認
        ...
        assert cache_metrics.cache_hit_ratio_improvement >= 0.30      # 30%以上キャッシュヒット率向上
        assert cache_metrics.range_cache_efficiency >= 0.85          # 85%以上範囲キャッシュ効率
        assert cache_metrics.intelligent_cache_score >= 0.80         # 80%以上インテリジェントキャッシュ

    def test_range_loading_integration_verification(self, range_lazy_components, structured_excel_file):
        """データ範囲遅延読み込み統合確認
        ...
        - スケーラビリティ確保
        """
        result = range_lazy_components['range_lazy_loader'].verify_range_loading_integration(
            structured_excel_file,
            {
...
class TestRangeBasedLazyLoadingEdgeCases:
    ...
    def test_invalid_range_specification_handling(self, range_lazy_components, structured_excel_file):
        """無効範囲指定エラーハンドリング"""
    ...
    def test_large_range_loading_optimization(self, range_lazy_components, multi_sheet_range_file):
        """大容量範囲読み込み最適化処理"""
    ...
    def test_multi_sheet_range_loading(self, range_lazy_components, multi_sheet_range_file):
        """複数シート範囲読み込み処理"""
        ...
        result = range_lazy_components['range_lazy_loader'].integrate_with_lazy_loading_foundation(
            multi_sheet_range_file,
            {
                ...
                "optimize_range_lazy_synergy": True,
                "target_sheets": ["DataSheet1", "DataSheet2"]  # 特定シートの範囲読み込み
            }
        )
        
        assert result.lazy_integration_success == True
        assert result.lazy_integration_metrics.lazy_integration_effectiveness >= LAZY_INTEGRATION_TARGET
