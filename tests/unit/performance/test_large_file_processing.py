...
class TestLargeFileProcessing:
    ...
    def create_large_test_file(self, target_size_mb: int = 100, filename: str = "large_test.xlsx") -> Path:
        """大容量テストファイル作成
        ...
        print(f"Creating large file: {target_size_mb}MB target, ~{estimated_rows:,} rows")
        
        # 一括でDataFrame作成（メモリ効率的な方法）
        all_data = {
            'ID': [f"ID{i:08d}" for i in range(estimated_rows)],
            'Name': [f"Name_{i}_with_long_content_for_size" for i in range(estimated_rows)],
            'Value': [i * 123.456789 for i in range(estimated_rows)],
            'Category': [f"Category_{i % 100}_with_extra_content" for i in range(estimated_rows)],
            'Description': [f"Description for item {i} with detailed information and extra content for size" for i in range(estimated_rows)],
            'Status': [f"Status_{i % 20}_active_inactive" for i in range(estimated_rows)],
            'Timestamp': [f"2025-01-{(i % 28) + 1:02d} 12:34:56" for i in range(estimated_rows)],
            'Score': [(i * 7) % 1000 for i in range(estimated_rows)],
            'Notes': [f"Notes for item {i} " * 3 for i in range(estimated_rows)],  # 長いテキスト
            'Metadata': [f"{{\"id\": {i}, \"type\": \"test\", \"extra\": \"data_content\"}}" for i in range(estimated_rows)]
        }
    ...
    def test_large_file_processing_100mb(self):
        ...
        print(f"Large file processing completed: {processing_time:.2f}s, {memory_increase/1024/1024:.1f}MB peak")
    ...
    def test_integrated_component_coordination(self):
        ...
        """
        # 中規模ファイル作成（協調動作テスト用）
        test_file = self.create_large_test_file(target_size_mb=50, filename="coordination_test.xlsx")
        ...
        chunk_processor = OptimizedChunkProcessor(chunk_size=2000, max_workers=2, enable_parallel_processing=True)
        ...
        assert coordination_efficiency['cross_component_synergy'] >= 1.1  # 10%以上シナジー効果
    ...
    def test_memory_pressure_handling(self):
        ...
        with memory_constrained_processor.simulate_memory_pressure():
            processing_result = memory_constrained_processor.process_under_pressure(large_file)
    ...
    def test_concurrent_large_file_processing(self):
        ...
        files = []
        for i in range(3):
            file_path = self.create_large_test_file(target_size_mb=40, filename=f"concurrent_test_{i}.xlsx")
        ...
        for i, file_path in enumerate(files):
            thread = threading.Thread(target=process_file_concurrently, args=(file_path, i))
    ...
    def test_error_recovery_large_files(self):
        ...
        """
        # 正常ファイルと破損ファイル作成
        normal_file = self.create_large_test_file(target_size_mb=60, filename="normal_large.xlsx")
        ...
        # 正常ファイル処理確認（回復能力テスト）
        recovery_result = error_recovery_processor.process_with_error_handling(normal_file)
    ...
    def test_performance_benchmarking(self):
        ...
        """
        # ベンチマーク用ファイル作成
        benchmark_file = self.create_large_test_file(target_size_mb=70, filename="benchmark_test.xlsx")
        ...
        # 従来処理ベンチマーク
        traditional_metrics = benchmarker.benchmark_traditional_processing(benchmark_file)
        
        # 最適化処理ベンチマーク
        optimized_metrics = benchmarker.benchmark_optimized_processing(benchmark_file)
        
        # パフォーマンス比較
        comparison = benchmarker.compare_performance(traditional_metrics, optimized_metrics)
        ...
        print(f"Performance improvement: {comparison.processing_time_improvement:.1f}x speed, {comparison.memory_usage_improvement:.1f}x memory efficiency")