...
class TestCachePerformanceIntegration:
    ...
    def create_performance_test_file(
        self, filename: str = "perf_test.xlsx", size_mb: int = 5
    ) -> Path:
        """パフォーマンステスト用ファイル作成
        ...
        data = {
            ...
            "Description": [
                f"Performance test data {i}" for i in range(estimated_rows)
            ],
    ...
    def test_cache_performance_comprehensive_analysis(self):
        ...
        cache_configurations = [
            ...
            (
                "basic_cache",
                CacheConfiguration(max_cache_size=5, enable_compression=False),
            ),
            (
                "optimized_cache",
                CacheConfiguration(max_cache_size=10, enable_compression=True),
            ),
        ]
        ...
        # 結果分析・比較
        comparison_analysis = performance_analyzer.analyze_performance_comparison(
            performance_results
        )
        ...
        assert (
            comparison_analysis.distributed_cache_overhead <= 0.3
        )  # 30%以下のオーバーヘッド

        print(
            f"Cache effectiveness: {comparison_analysis.cache_effectiveness_score:.2f}"
        )
    ...
    def test_cache_scalability_analysis(self):
        ...
        for size_label, size_mb in data_sizes:
            test_file = self.create_performance_test_file(
                f"scalability_{size_label}.xlsx", size_mb
            )
            size_results = {}
        ...
        # スケーラビリティ分析
        scaling_analysis = scalability_analyzer.analyze_scalability_characteristics(
            scalability_results
        )

        # スケーラビリティ結果検証
        assert (
            scaling_analysis.linear_scaling_efficiency >= 0.7
        )  # 70%以上の線形スケーリング効率
    ...
    def test_cache_effectiveness_detailed_report(self):
        ...
        cache_scenarios = [
            ...
            (
                "distributed_cache_small",
                DistributedCacheConfiguration(cache_node_count=2, replication_factor=1),
            ),
            (
                "distributed_cache_large",
        ...
        assert (
            tech_analysis.throughput_improvement >= 1.5
        )  # 1.5倍以上のスループット向上
    ...
    def test_cache_integration_pipeline_performance(self):
        ...
        # パイプライン統合テスト用ファイル
        pipeline_test_file = self.create_performance_test_file(
            "pipeline_test.xlsx", 120
        )
        ...
        # パイプライン統合分析
        pipeline_analysis = (
            pipeline_analyzer.analyze_pipeline_integration_effectiveness(
                integration_results
            )
        )
    ...
    def test_cache_performance_regression_detection(self):
        ...
        # 回帰検出用テストファイル
        regression_test_file = self.create_performance_test_file(
            "regression_test.xlsx", 90
        )
        ...
        regression_analysis = regression_analyzer.analyze_regression_patterns(
            baseline_performance, regression_results
        )
        ...
        memory_alert = next(
            (alert for alert in alerts if "memory" in alert.alert_type), None
        )
        ...
        assert any(
            "cache_size" in suggestion.target for suggestion in improvement_suggestions
        )
