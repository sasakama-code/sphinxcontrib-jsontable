...
class TestMockExcelReaderInitialization:
    ...
    def test_init_custom_configuration(self):
        ...
        assert mock_reader.mock_sheet_names == ["カスタムシート1", "カスタムシート2", "カスタムシート3"]
...
class TestMockExcelReaderFileValidation:
    ...
    def test_validate_file_path_handling(self):
        ...
        assert mock_reader.validate_file_calls[0]["file_path"] == "relative/path/file.xlsx"
        assert mock_reader.validate_file_calls[1]["file_path"] == "/absolute/path/file.xlsx"
...
class TestMockExcelReaderCallTracking:
    ...
    def test_call_tracking_accumulation(self):
        ...
        # 各コールの記録確認
        assert len(mock_reader.validate_file_calls) == 3  # file1, file3, file4 (read_workbook内部で呼ばれる)
        assert len(mock_reader.get_sheet_names_calls) == 1  # file2
        assert len(mock_reader.read_workbook_calls) == 2  # file3 (直接) + file4 (read_sheet経由)
    ...
    def test_reset_call_tracking_functionality(self):
        ...
        assert summary_before["total_calls"] == 4  # validate_file(2回), get_sheet_names, read_workbook
...
class TestMockExcelReaderIntegration:
    ...
    def test_complex_workflow_simulation(self):
        ...
        # 4. 統合結果確認
        summary = mock_reader.get_call_summary()
        assert summary["total_calls"] == 4  # validate_file(2回), get_sheet_names(1回), read_workbook(1回)
    ...
    def test_error_consistency_across_methods(self):
        ...
        # エラー時でもコールトラッキングが機能
        summary = mock_reader.get_call_summary()
        assert summary["total_calls"] == 4  # validate_file, get_sheet_names, read_workbook, read_sheet
