...
class TestUnifiedErrorHandling:
    ...
    def create_test_excel_file(self, filename: str = "test_error_handling.xlsx") -> Path:
        """テスト用Excelファイル作成
    ...
    def test_error_deduplication_across_pipelines(self):
        ...
        print(f"Deduplication efficiency: {deduplication_stats['deduplication_efficiency']:.1%}")
    ...
    def test_cross_pipeline_error_recovery(self):
        ...
        print(f"Cross-pipeline recoveries: {cross_pipeline_stats['pipeline_collaboration_count']}")
    ...
    def test_unified_error_classification(self):
        ...
        # 多様なエラータイプのテストケース
        test_errors = [
            (FileNotFoundError("File not found"), ErrorCategory.FILE_ACCESS, ErrorSeverity.HIGH),
            (ValueError("Invalid range: A1:Z999"), ErrorCategory.RANGE_VALIDATION, ErrorSeverity.MEDIUM),
            (KeyError("Column 'Name' not found"), ErrorCategory.HEADER_PROCESSING, ErrorSeverity.MEDIUM),
            (PermissionError("Access denied"), ErrorCategory.SECURITY_VALIDATION, ErrorSeverity.CRITICAL),
            (TypeError("Cannot convert data type"), ErrorCategory.DATA_CONVERSION, ErrorSeverity.HIGH),
            (ConnectionError("Cache server unreachable"), ErrorCategory.PIPELINE_INTEGRATION, ErrorSeverity.LOW)
        ]
        ...
        for error, expected_category, expected_severity in test_errors:
            ...
            classification_results.append((classification, expected_category, expected_severity))
        
        # 分類精度検証
        correct_categories = 0
        correct_severities = 0
        
        for classification, expected_category, expected_severity in classification_results:
        ...
        assert classification_stats['category_distribution'][ErrorCategory.FILE_ACCESS] >= 1
    ...
    def test_integrated_error_monitoring(self):
        ...
        for i, error in enumerate(error_events):
            classification = PipelineErrorClassification(
                error_category=ErrorCategory.FILE_ACCESS if isinstance(error, (FileNotFoundError, PermissionError)) else ErrorCategory.RANGE_VALIDATION,
                severity_level=ErrorSeverity.HIGH if isinstance(error, (FileNotFoundError, PermissionError)) else ErrorSeverity.MEDIUM,
                pipeline_stage=PipelineStage.DATA_ACQUISITION,
                recovery_priority=1,
                cross_pipeline_impact=True
            )
        ...
        assert monitoring_stats['errors_by_category'][ErrorCategory.RANGE_VALIDATION] >= 2
    ...
    def test_performance_impact_minimal(self):
        ...
        print(f"Average handling time: {efficiency_stats['average_handling_time_ms']:.2f}ms")
    ...
    def test_unified_recovery_strategies(self):
        ...
        print(f"Cross-pipeline fallbacks: {cross_pipeline_results['successful_fallbacks']}")