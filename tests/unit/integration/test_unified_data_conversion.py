...
class TestUnifiedDataConversion:
    ...
    def create_test_excel_file(
        self, filename: str = "test_data_conversion.xlsx"
    ) -> Path:
        """テスト用Excelファイル作成
    ...
    def test_unified_data_conversion_engine_initialization(self):
        ...
        assert isinstance(
            conversion_engine.performance_analyzer, ConversionPerformanceAnalyzer
        )
    ...
    def test_duplication_elimination_across_pipelines(self):
        ...
        # 統合データ変換エンジン初期化
        conversion_engine = UnifiedDataConversionEngine(
            enable_deduplication=True, enable_performance_optimization=True
        )
        ...
        print(
            f"Performance improvement: {optimized_result.performance_improvement:.1%}"
        )
    ...
    def test_single_pass_data_conversion(self):
        ...
        print(
            f"Memory peak usage: {performance_stats.memory_peak_usage / 1024 / 1024:.1f}MB"
        )
    ...
    def test_conversion_stage_metrics_analysis(self):
        ...
        # 統合データ変換エンジン（分析付き）
        conversion_engine = UnifiedDataConversionEngine(
            performance_analyzer=performance_analyzer, enable_detailed_metrics=True
        )
        ...
        for stage_metric in stage_metrics:
            ...
            assert stage_metric.stage_name in [
                "data_acquisition",
                "unified_conversion",
                "result_construction",
            ]
        ...
        print(
            f"Performance improvement: {baseline_comparison.performance_improvement:.1%}"
        )
    ...
    def test_conversion_caching_integration(self):
        ...
        # キャッシュ統合変換エンジン
        conversion_engine = UnifiedDataConversionEngine(
            cache_manager=cache_manager, enable_caching=True
        )
        ...
        assert (
            second_result.processing_time_ms < first_result.processing_time_ms * 0.1
        )  # 90%高速化
        ...
        # ファイル変更検出テスト
        # ファイル更新
        updated_data = {"ID": [1, 2], "Name": ["Alice", "Bob"], "Score": [95.5, 87.2]}
        ...
        print(
            f"Processing time reduction: {(1 - second_result.processing_time_ms / first_result.processing_time_ms):.1%}"
        )
    ...
    def test_data_type_optimization_integration(self):
        ...
        assert (
            type_results["mixed_type_optimization"]["strategy_applied"]
            == "smart_conversion"
        )
        ...
        print(
            f"Total memory reduction: {memory_improvement.total_memory_reduction:.1%}"
        )
        print(
            f"Type conversion accuracy: {quality_metrics.type_conversion_accuracy:.1%}"
        )
