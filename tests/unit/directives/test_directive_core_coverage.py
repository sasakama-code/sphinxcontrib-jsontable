...
class TestJsonTableDirectiveGetProcessor:
    ...
    def test_get_processor_excel_file(self):
        ...
        with patch('sphinxcontrib.jsontable.directives.directive_core.ExcelProcessor') as mock_excel_processor:
            processor = self.directive.get_processor()
...
class TestJsonTableDirectiveErrorHandling:
    ...
    def test_run_with_table_conversion_error(self):
        ...
        with patch.object(self.directive, 'get_processor') as mock_get_processor, \
             patch('sphinxcontrib.jsontable.directives.directive_core.TableConverter') as mock_converter:
...
class TestJsonTableDirectiveIntegration:
    ...
    def test_run_complete_flow_json_inline(self):
        """インラインJSON処理の完全フロー"""
        self.directive.content = ['[{"name": "Alice", "age": 30}, {"name": "Bob", "age": 25}]']
        self.directive.options = {'header': None}
        
        with patch('sphinxcontrib.jsontable.directives.directive_core.TableConverter') as mock_converter, \
             patch('sphinxcontrib.jsontable.directives.directive_core.TableBuilder') as mock_builder:
...
class TestJsonTableDirectiveCompatibility:
    ...
    def test_default_values_consistency(self):
        ...
        from sphinxcontrib.jsontable.directives.backward_compatibility import DEFAULT_MAX_ROWS, DEFAULT_ENCODING
