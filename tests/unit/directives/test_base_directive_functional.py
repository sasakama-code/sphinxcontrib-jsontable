...
class TestBaseDirectiveTemplateMethodPattern:
    ...
    def directive_instance(self, sphinx_app_mock):
        ...
        # テーブルビルダーのモック設定
        with patch('sphinxcontrib.jsontable.directives.base_directive.TableBuilder') as mock_table_builder:
    ...
    def test_template_method_execution_flow_success(self, directive_instance):
        ...
        with patch.object(directive_instance, '_validate_execution_context') as mock_validate:
            with patch.object(directive_instance, '_process_options') as mock_process:
                with patch.object(directive_instance, '_validate_and_convert_data') as mock_convert:
                    mock_convert.return_value = [["Header1", "Header2"], ["Value1", "Value2"]]
    ...
    def test_abstract_method_delegation_data_loading(self, directive_instance):
        ...
        with patch.object(directive_instance, '_load_data', return_value=[["test"]]) as mock_load:
            with patch.object(directive_instance, '_validate_and_convert_data', return_value=[["test"]]):
                result = directive_instance.run()
...
class TestBaseDirectiveErrorHandling:
    ...
    def test_unexpected_error_security_sanitization(self):
        ...
        class UnexpectedErrorDirective(BaseDirective):
            ...
            def _load_data(self):
                raise ValueError("Internal system error with sensitive data: secret_key_123")
        ...
        with patch('sphinxcontrib.jsontable.directives.base_directive.TableBuilder'):
            ...
            # セキュリティサニタイゼーションの確認
            error_text = str(result[0])
            assert "secret_key_123" not in error_text  # 機密情報が含まれていないことを確認
