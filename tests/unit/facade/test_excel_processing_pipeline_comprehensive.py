...
class TestSecurityValidationStage:
    ...
    def test_security_validation_threat_detection(self, pipeline, mock_components):
        ...
        mock_components['error_handler'].create_error_response.return_value = "Security error response"
...
class TestRangeParsingStage:
    ...
    def test_range_parsing_error_handling(self, pipeline, mock_components):
        ...
        mock_components['error_handler'].create_error_response.return_value = "Range error response"
    ...
    def test_range_parsing_error_without_handler(self, disabled_pipeline, mock_components):
        ...
        - システムの安定性
        """
        mock_components['range_parser'].parse.side_effect = ValueError("Invalid range format")
...
class TestFileReadingStage:
    ...
    def test_excel_file_reading_success(self, pipeline, mock_components):
        ...
        mock_components['excel_reader'].read_workbook.return_value = expected_read_result
        
        result = pipeline._read_excel_file(
            "test.xlsx", "Sheet1", None, "test_context"
        )
    ...
    def test_excel_file_reading_error_handling(self, pipeline, mock_components):
        ...
        - 診断情報の有用性
        """
        mock_components['excel_reader'].read_workbook.side_effect = Exception("File not found")
        mock_components['error_handler'].create_error_response.return_value = "File error response"
...
class TestDataConversionStage:
    ...
    def test_data_conversion_success(self, pipeline, mock_components):
        ...
        mock_components['data_converter'].convert_dataframe_to_json.return_value = expected_conversion_result
        
        result = pipeline._convert_data_to_json(test_df, 0, "test_context")
        
        assert result == expected_conversion_result
        mock_components['data_converter'].convert_dataframe_to_json.assert_called_once_with(
            test_df, header_row=0
        )
    ...
    def test_data_conversion_error_handling(self, pipeline, mock_components):
        ...
        mock_components['data_converter'].convert_dataframe_to_json.side_effect = Exception("Conversion error")
        mock_components['error_handler'].create_error_response.return_value = "Conversion error response"
...
class TestResultIntegrationStage:
    ...
    def test_result_integration_error_handling(self, pipeline, mock_components):
        ...
        mock_components['error_handler'].create_error_response.return_value = "Integration error response"
...
class TestEndToEndProcessing:
    ...
    def test_complete_pipeline_success(self, pipeline, mock_components):
        ...
        """
        # モック設定
        mock_components['security_validator'].validate_file.return_value = {"is_safe": True}
        mock_components['range_parser'].parse.return_value = RangeInfo(
            start_row=0, start_col=0, end_row=1, end_col=1,
            original_spec="A1:B2", normalized_spec="A1:B2"
        )
        mock_components['excel_reader'].read_workbook.return_value = ReadResult(
            dataframe=pd.DataFrame({'Name': ['Alice'], 'Age': [25]}),
            workbook_info=WorkbookInfo(
                file_path=Path("test.xlsx"), sheet_names=["Sheet1"],
                has_macros=False, has_external_links=False, file_size=1024, format_type=".xlsx"
            ),
            metadata={"sheet_name": "Sheet1"}
        )
        mock_components['data_converter'].convert_dataframe_to_json.return_value = ConversionResult(
            data=[['Alice', 25]], has_header=True, headers=['Name', 'Age'],
            metadata={'conversion_type': 'dataframe_to_json'}
        )
    ...
    def test_complete_pipeline_with_error_recovery(self, pipeline, mock_components):
        ...
        - システムの安定性
        """
        mock_components['excel_reader'].read_workbook.side_effect = Exception("File error")
