...
class TestExcelProcessingPipelineSecurityStage:
    ...
    def test_security_validation_threat_detection_and_blocking(
        ...
        with pytest.raises(SecurityError) as exc_info:
            pipeline._perform_security_validation(
                "/test/malicious_file.xlsx", "test_context"
            )
...
class TestExcelProcessingPipelineRangeStage:
    ...
    def test_range_parsing_success_valid_specification(
        self, pipeline_with_range_parser
    ):
        """
        有効な範囲指定の解析成功を検証する。
    ...
    def test_range_parsing_error_handling_invalid_specification(
        self, pipeline_with_range_parser
    ):
        """
        ...
        components[
            "error_handler"
        ].create_error_response.return_value = "Range parsing error"
...
class TestExcelProcessingPipelineFileReadingStage:
    ...
    def test_excel_file_reading_success_with_sheet_name(
        self, pipeline_with_excel_reader
    ):
        """
    ...
    def test_excel_file_reading_success_with_sheet_index(
        self, pipeline_with_excel_reader
    ):
        """
        ...
        # シートインデックス指定での読み込み
        result = pipeline._read_excel_file("/test/file.xlsx", None, 1, "test_context")
    ...
    def test_excel_file_reading_error_handling_file_not_found(
        self, pipeline_with_excel_reader
    ):
        """
        ...
        components[
            "error_handler"
        ].create_error_response.return_value = "File reading error"
...
class TestExcelProcessingPipelineDataConversionStage:
    ...
    def test_data_conversion_success_with_header_row(
        self, pipeline_with_data_converter
    ):
        """
        ...
        components[
            "data_converter"
        ].convert_dataframe_to_json.return_value = mock_conversion_result
    ...
    def test_data_conversion_error_handling_invalid_data(
        self, pipeline_with_data_converter
    ):
        """
        ...
        components[
            "data_converter"
        ].convert_dataframe_to_json.side_effect = conversion_error

        # エラーハンドラーのモック設定
        components[
            "error_handler"
        ].create_error_response.return_value = "Conversion error"
...
class TestExcelProcessingPipelineResultIntegration:
    ...
    def test_result_integration_success_with_range_info(self, pipeline_for_integration):
        ...
        with patch("pandas.Timestamp") as mock_timestamp:
            mock_timestamp.now.return_value.isoformat.return_value = (
                "2023-01-01T12:00:00"
            )
    ...
    def test_result_integration_success_without_range_info(
        self, pipeline_for_integration
    ):
        """
        範囲情報なし結果統合の成功を検証する。
