...
class TestSecurityComprehensiveCoverage:
    ...
    def create_comprehensive_security_excel(self, filename="comprehensive_security.xlsx"):
    ...
    def test_external_link_security_strict_mode_comprehensive(self):
        ...
        try:
            ...
            security_keywords = [
                "dangerous", "external", "security", "file://", "javascript", 
                "vbscript", "ftp://", "ldap://", "data:", "links", "detected"
            ]
    ...
    def test_external_link_security_warn_mode_comprehensive(self):
        ...
        if warning_list:
            ...
            security_warnings = [
                ...
                if any(keyword in msg for keyword in ["security", "warning", "dangerous", "external"])
            ]
            # セキュリティ警告があれば検証
    ...
    def test_external_link_security_allow_mode_comprehensive(self):
        ...
        security_warnings = [
            ...
            if "security" in str(w.message).lower() or "dangerous" in str(w.message).lower()
        ]
    ...
    def test_macro_security_validation_comprehensive(self):
        ...
        try:
            ...
            if warning_list:
                ...
                macro_warnings = [
                    msg for msg in warning_messages 
                    if any(keyword in msg.lower() for keyword in ["macro", "xlsm", "security"])
                ]
                if macro_warnings:
            ...
            # マクロセキュリティエラーが発生した場合
            error_msg = str(e).lower()
            assert any(keyword in error_msg for keyword in ["macro", "xlsm", "security"])
        except Exception:
            ...
            try:
                ...
                if mode == "strict":
                    ...
                    if warning_list:
                        ...
                        [
                            msg for msg in warning_messages 
                            if any(kw in msg for kw in ["security", "dangerous", "warning"])
                        ]
                elif mode == "allow":
                    # allowモードではセキュリティ警告なし
                    security_warnings = [
                        w for w in warning_list 
                        if any(kw in str(w.message).lower() for kw in ["security", "dangerous"])
                    ]
                ...
                if mode == "strict":
                    error_msg = str(e).lower()
                    assert any(kw in error_msg for kw in ["security", "dangerous", "external"])
                else:
    ...
    def test_hyperlink_and_cell_content_detection_comprehensive(self):
        ...
        for loader in [self.loader_strict, self.loader_warn, self.loader_allow]:
            try:
                ...
                error_msg = str(e).lower()
                expected_keywords = [
                    "dangerous", "external", "links", "detected", "security",
                    "file://", "javascript", "vbscript", "ftp://", "ldap://", "data:"
                ]
