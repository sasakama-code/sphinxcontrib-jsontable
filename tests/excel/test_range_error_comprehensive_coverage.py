...
class TestRangeErrorComprehensiveCoverage:
    ...
    def create_large_test_excel(self, filename="large_range_test.xlsx", rows=50, cols=30):
        """大きなテスト用Excelファイル作成."""
    ...
    def test_cell_address_parsing_error_comprehensive(self):
        ...
        for invalid_range in invalid_cell_addresses:
            try:
                result = self.loader.load_from_excel_with_range(excel_path, invalid_range)
    ...
    def test_range_bounds_validation_comprehensive(self):
        ...
        for range_spec in out_of_bounds_ranges:
            try:
                ...
                error_message = str(e)
                boundary_keywords = [
                    "exceeds", "bounds", "range", "invalid", "data", "size",
                    "rows", "columns", "boundary", "out of"
                ]
                assert any(keyword in error_message.lower() for keyword in boundary_keywords)
    ...
    def test_range_specification_whitespace_handling_comprehensive(self):
        ...
        for range_pattern in whitespace_patterns:
            try:
                ...
                result = self.loader.load_from_excel_with_range(excel_path, range_pattern)
                assert isinstance(result, dict)
                assert "data" in result
            except RangeSpecificationError as e:
                # 一部の空白パターンではエラーになる
    ...
    def test_range_specification_original_spec_preservation(self):
        ...
        for range_spec in valid_ranges:
            ...
            # メタデータが含まれている場合の確認
            if "metadata" in result or "range_info" in result or "original_spec" in result:
                # 元の範囲指定が保持されているかチェック
                pass
