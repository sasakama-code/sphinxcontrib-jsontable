...
class TestErrorHandlingComprehensiveCoverage:
    ...
    def test_file_not_found_error_handling_comprehensive(self):
        ...
        for filename in non_existent_files:
            ...
            with pytest.raises((ExcelFileNotFoundError, FileNotFoundError, OSError, ValueError)):
        ...
        error_conditions = [
            {
                ...
                "range": "INVALID",
                "expected_error": (ExcelFileNotFoundError, FileNotFoundError, RangeSpecificationError),
            },
            {
                "description": "Empty filename with None range",
                "file": "",
                "range": None,
                "expected_error": (ExcelFileNotFoundError, FileNotFoundError, TypeError, ValueError),
            },
            {
        ...
        for condition in error_conditions:
            with pytest.raises(condition["expected_error"]):
                if condition["range"] is not None:
                    self.loader.load_from_excel_with_range(condition["file"], condition["range"])
                else:
            ...
            try:
                if test_case["operation"] == "load_with_nonexistent_file":
                    ...
                    self.loader.load_from_excel_with_range(
                        test_case["params"]["file"],
                        test_case["params"]["range"]
                    )
            except Exception as e:
                # 例外が適切に処理されていることを確認
