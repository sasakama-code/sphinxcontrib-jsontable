...
from sphinxcontrib.jsontable.enhanced_directive import EnhancedJsonTableDirective, RAGProcessingResult
...
class TestEnhancedJsonTableDirective:
    ...
    def test_file_based_input(self):
        ...
        rag_result = directive._process_rag_pipeline(json_data)
        assert rag_result.basic_metadata is not None
        assert "エンジニア" in rag_result.basic_metadata.search_keywords or \
               "山田太郎" in rag_result.basic_metadata.search_keywords
    ...
    def test_chunk_strategies(self):
        ...
        for strategy in strategies:
            ...
            assert len(rag_result.semantic_chunks) > 0
            # スキーマチャンクが含まれることを確認
            schema_chunks = [c for c in rag_result.semantic_chunks if c.chunk_type == "schema"]
    ...
    def test_export_format_parsing(self):
        ...
        options_tests = [
            ...
            ({'export-formats': ' opensearch , elasticsearch '}, ['opensearch', 'elasticsearch']),
            ({}, [])
        ]
