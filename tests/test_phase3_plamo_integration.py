...
from sphinxcontrib.jsontable.rag.vector_processor import (
    PLaMoVectorProcessor, VectorChunk, VectorProcessingResult,
    JapaneseTextNormalizer, BusinessTermEnhancer, ContextPreserver
)
from sphinxcontrib.jsontable.rag.search_index_generator import (
    SearchIndexGenerator, ComprehensiveSearchIndex, VectorIndex,
    SemanticSearchIndex, FacetedSearchIndex, HybridSearchIndex,
    JapaneseQueryProcessor
)
from sphinxcontrib.jsontable.rag.query_processor import (
    IntelligentQueryProcessor, QueryAnalysis, SearchResult,
    QueryExecutionResult, QueryIntentClassifier, HybridSearchEngine
)
...
def basic_metadata_sample():
    ...
    return BasicMetadata(
        ...
        extraction_timestamp="2025-06-07T10:00:00"
    )
...
class TestPLaMoVectorProcessor:
    ...
    @pytest.mark.asyncio  
    async def test_japanese_enhancement_application(self, semantic_chunks_from_business_data):
        """日本語強化適用テスト"""
        processor = PLaMoVectorProcessor()
    ...
    @pytest.mark.asyncio
    async def test_batch_processing_performance(self, semantic_chunks_from_business_data):
        """バッチ処理性能テスト"""
        processor = PLaMoVectorProcessor()
...
class TestSearchIndexGenerator:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_index_generation(self, semantic_chunks_from_business_data, basic_metadata_sample):
        """包括的インデックス生成テスト"""
        # まずベクトル処理
        vector_processor = PLaMoVectorProcessor()
        vector_result = await vector_processor.process_chunks(semantic_chunks_from_business_data)
    ...
    @pytest.mark.asyncio
    async def test_semantic_index_construction(self, semantic_chunks_from_business_data):
        """セマンティックインデックス構築テスト"""
        vector_processor = PLaMoVectorProcessor()
        vector_result = await vector_processor.process_chunks(semantic_chunks_from_business_data)
    ...
    @pytest.mark.asyncio
    async def test_faceted_index_construction(self, semantic_chunks_from_business_data, basic_metadata_sample):
        """ファセットインデックス構築テスト"""
        vector_processor = PLaMoVectorProcessor()
        vector_result = await vector_processor.process_chunks(semantic_chunks_from_business_data)
        
        generator = SearchIndexGenerator()
        facet_index = generator._build_facet_index(vector_result.vector_chunks, basic_metadata_sample)
...
class TestQueryIntentClassifier:
    ...
    def test_intent_classification(self):
        ...
        assert intent in ["faceted_search", "numerical_search"]
...
class TestIntelligentQueryProcessor:
    ...
    @pytest.mark.asyncio
    async def test_complete_query_processing_pipeline(self, semantic_chunks_from_business_data, basic_metadata_sample):
        """完全なクエリ処理パイプラインテスト"""
        # Phase 3コンポーネント統合
        vector_processor = PLaMoVectorProcessor()
        vector_result = await vector_processor.process_chunks(semantic_chunks_from_business_data)
    ...
    @pytest.mark.asyncio
    async def test_japanese_business_query_optimization(self, semantic_chunks_from_business_data, basic_metadata_sample):
        """日本語ビジネスクエリ最適化テスト"""
        vector_processor = PLaMoVectorProcessor()
        vector_result = await vector_processor.process_chunks(semantic_chunks_from_business_data)
    ...
    def test_search_suggestions(self, semantic_chunks_from_business_data, basic_metadata_sample):
        """検索サジェストテスト"""
        # 簡易インデックス作成
...
class TestPhase3Integration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_japanese_rag_pipeline(self, sample_japanese_business_data):
        ...
        # Step 3: 検索インデックス構築
        generator = SearchIndexGenerator()
        search_index = generator.generate_comprehensive_index(vector_result.vector_chunks)
        ...
        # パフォーマンス検証
        avg_execution_time = sum(r["execution_time"] for r in results_summary) / len(results_summary)
        assert avg_execution_time < 3000  # 平均3秒以内
    
    @pytest.mark.asyncio
    async def test_phase3_performance_benchmarks(self, semantic_chunks_from_business_data):
