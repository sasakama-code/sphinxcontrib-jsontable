...
try:
    ...
    logging.warning("FAISS not available. Vector search will use fallback implementation.")
...
class VectorIndex:
    ...
    faiss_index: Optional[Any] = None  # FAISSインデックス
    fallback_embeddings: Optional[np.ndarray] = None  # フォールバック用埋め込み
...
class ComprehensiveSearchIndex:
    ...
    semantic_index: Optional[SemanticSearchIndex] = None
    facet_index: Optional[FacetedSearchIndex] = None
    hybrid_index: Optional[HybridSearchIndex] = None
...
class SearchIndexGenerator:
    ...
    def generate_comprehensive_index(
        ...
        logger.info(f"Generating comprehensive search index for {len(vector_chunks)} chunks")
        ...
        # 3. ファセット検索インデックス
        search_index.facet_index = self._build_facet_index(vector_chunks, basic_metadata)
        ...
        search_index.index_statistics = {
            ...
            'semantic_terms_indexed': len(search_index.semantic_index.japanese_keyword_index),
    ...
    def _build_vector_index(self, vector_chunks: List[VectorChunk]) -> VectorIndex:
        ...
        if FAISS_AVAILABLE:
            ...
            logger.info(f"Fallback vector index built with {len(vector_chunks)} vectors")
        
        return vector_index
    
    def _build_semantic_index(self, vector_chunks: List[VectorChunk]) -> SemanticSearchIndex:
        """セマンティック検索インデックス構築"""
        ...
        # セマンティックマッピング構築
        semantic_index.semantic_mappings = self._build_semantic_mappings(
            vector_chunks
        )
        
        logger.info(f"Semantic index built with {len(semantic_index.japanese_keyword_index)} keywords")
        
        return semantic_index
    
    def _build_japanese_keyword_index(self, vector_chunks: List[VectorChunk]) -> Dict[str, List[int]]:
        """日本語キーワードインデックス構築"""
    ...
    def _build_business_term_index(self, vector_chunks: List[VectorChunk]) -> Dict[str, List[int]]:
        """ビジネス用語インデックス構築"""
    ...
    def _build_semantic_mappings(self, vector_chunks: List[VectorChunk]) -> Dict[str, List[int]]:
        """セマンティックマッピング構築"""
    ...
    def _build_facet_index(
        ...
        # 時間ファセット構築
        facet_index.temporal_facets = self._build_temporal_facets(
            vector_chunks
        )
        
        # エンティティファセット構築
        facet_index.entity_facets = self._build_entity_facets(
            vector_chunks
        )
        
        logger.info(f"Faceted index built with {len(facet_index.categorical_facets)} categorical facets")
    ...
    def _build_categorical_facets(
        ...
        business_categories = ['financial', 'organizational', 'operational', 'strategic']
        
        for category in business_categories:
            categorical_facets['business_category'][category] = []
            
            for i, chunk in enumerate(vector_chunks):
                if self._categorize_business_content(chunk.original_chunk.content, category):
    ...
    def _build_temporal_facets(self, vector_chunks: List[VectorChunk]) -> Dict[str, Dict[str, List[int]]]:
        """時間ファセット構築"""
        ...
        quarters = ['Q1', 'Q2', 'Q3', 'Q4', '第1四半期', '第2四半期', '第3四半期', '第4四半期']
    ...
    def _build_entity_facets(self, vector_chunks: List[VectorChunk]) -> Dict[str, Dict[str, List[int]]]:
        """エンティティファセット構築"""
    ...
    def _categorize_business_content(self, content: str, category: str) -> bool:
        ...
        category_keywords = {
            'financial': ['売上', '利益', '収益', '売上高', '純利益', '営業利益', '財務', '予算'],
            'organizational': ['組織', '部門', '人事', '従業員', '社員', '採用', '研修'],
    ...
    def save_search_index(self, search_index: ComprehensiveSearchIndex, output_path: str):
        """検索インデックス保存"""
        ...
        # FAISSインデックス保存
        if FAISS_AVAILABLE and search_index.vector_index and search_index.vector_index.faiss_index:
        ...
        index_data = {
            ...
            'vector_index_metadata': {
                'chunk_metadata': search_index.vector_index.chunk_metadata if search_index.vector_index else [],
                'search_parameters': search_index.vector_index.search_parameters if search_index.vector_index else {},
                'dimension': search_index.vector_index.dimension if search_index.vector_index else 1024,
                'index_type': search_index.vector_index.index_type if search_index.vector_index else 'unknown'
            },
            'semantic_index': {
                'text_segments': search_index.semantic_index.text_segments if search_index.semantic_index else [],
                'japanese_keyword_index': search_index.semantic_index.japanese_keyword_index if search_index.semantic_index else {},
                'business_term_index': search_index.semantic_index.business_term_index if search_index.semantic_index else {},
                'semantic_mappings': search_index.semantic_index.semantic_mappings if search_index.semantic_index else {}
            },
            'facet_index': {
                'categorical_facets': search_index.facet_index.categorical_facets if search_index.facet_index else {},
                'numerical_facets': search_index.facet_index.numerical_facets if search_index.facet_index else {},
                'temporal_facets': search_index.facet_index.temporal_facets if search_index.facet_index else {},
                'entity_facets': search_index.facet_index.entity_facets if search_index.facet_index else {}
            },
            'hybrid_index': {
                'vector_weight': search_index.hybrid_index.vector_weight if search_index.hybrid_index else 0.7,
                'semantic_weight': search_index.hybrid_index.semantic_weight if search_index.hybrid_index else 0.2,
                'facet_weight': search_index.hybrid_index.facet_weight if search_index.hybrid_index else 0.1,
                'fusion_algorithm': search_index.hybrid_index.fusion_algorithm if search_index.hybrid_index else 'rank_fusion'
            }
        }
        
        # フォールバック埋め込み保存
        if search_index.vector_index and search_index.vector_index.fallback_embeddings is not None:
    ...
    def load_search_index(cls, input_path: str) -> ComprehensiveSearchIndex:
        ...
        search_index.semantic_index = SemanticSearchIndex(
            ...
            japanese_keyword_index=index_data['semantic_index']['japanese_keyword_index'],
