...
class RAGMetadataExtractor:
    ...
    def _init_japanese_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'name_indicators': [
                '名前', 'name', '氏名', '姓名', 'full_name', 'fullname',
                '担当者', 'person', '社員', 'employee', 'staff'
            ],
            'date_indicators': [
                '日付', 'date', '年月日', '時間', 'time', 'datetime',
                '作成日', 'created', '更新日', 'updated', '登録日'
            ],
            'money_indicators': [
                '価格', 'price', '金額', 'amount', '料金', 'fee', 'cost',
                '給与', 'salary', '売上', 'sales', '予算', 'budget'
            ],
            'organization_indicators': [
                '会社', 'company', '部署', 'department', '課', 'section',
                '組織', 'organization', 'team', 'チーム', '事業部'
            ]
        }
    ...
    def _init_type_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'number_with_unit_pattern': re.compile(r'^\d+(\.\d+)?(円|¥|%|個|件|人|歳)$')
        }
    ...
    def extract(self, json_data: JsonData, options: Dict[str, Any]) -> BasicMetadata:
        ...
        try:
            ...
            raise ValueError(f"メタデータ抽出に失敗しました: {e}")
    ...
    def _analyze_property(self, key: str, data_list: List[Any], sample_value: Any) -> Dict[str, Any]:
        """プロパティの詳細分析"""
    ...
    def _infer_semantic_type(self, key: str, value: Any) -> Optional[str]:
        ...
        for pattern in self.japanese_patterns['name_indicators']:
            if pattern in key_lower:
                if isinstance(value, str) and self.type_inference_patterns['japanese_name_pattern'].match(value):
    ...
    def _calculate_numeric_stats(self, key: str, data_list: List[Any]) -> Optional[Dict[str, Any]]:
        """数値プロパティの統計を計算"""
        try:
            values = []
    ...
    def _estimate_entity_type(self, columns: List[str]) -> Optional[str]:
        ...
        # 従業員・人事データ
        if any(keyword in column_text for keyword in ['name', '名前', 'employee', '社員', 'staff', '氏名']):
            if any(keyword in column_text for keyword in ['salary', '給与', 'department', '部署']):
                return "従業員"
        
        # 商品・製品データ
        if any(keyword in column_text for keyword in ['product', '商品', 'price', '価格', 'category', 'カテゴリ']):
            return "商品"
        
        # 売上・取引データ
        if any(keyword in column_text for keyword in ['sales', '売上', 'amount', '金額', 'date', '日付']):
            return "売上"
        
        # 顧客データ
        if any(keyword in column_text for keyword in ['customer', '顧客', 'client', 'contact', '連絡先']):
            return "顧客"
    ...
    def _extract_search_keywords(self, data: JsonData, schema: Dict[str, Any]) -> List[str]:
        ...
        # エンティティタイプベースのキーワード
        entity_type = self._estimate_entity_type(list(data[0].keys()) if isinstance(data, list) and data and isinstance(data[0], dict) else [])
        if entity_type:
        ...
        if isinstance(data, list):
            ...
            if data and isinstance(data[0], dict):
                ...
                for key in data[0].keys():
                    ...
                    # 完全性（非NULL率）
                    completeness[key] = len(non_null_values) / len(values) if values else 0
        ...
        for key, value in record.items():
            if value is not None:
                ...
                if isinstance(value, str):
                    ...
                    parts.append(f"{readable_key}={str(value)}")
