...
from .vector_processor import PLaMoVectorProcessor, VectorChunk
from .search_index_generator import (
    SearchIndexGenerator, ComprehensiveSearchIndex, 
...
class QueryIntentClassifier:
    ...
    def __init__(self):
        ...
        self.intent_patterns = {
            ...
            'faceted_search': {
                'patterns': [r'絞り込み', r'条件', r'フィルター', r'範囲', r'where', r'filter'],
                'description': 'ファセット検索'
            },
            'semantic_search': {
    ...
    def classify_business_context(self, query: str) -> Optional[str]:
        """ビジネス文脈分類"""
...
class HybridSearchEngine:
            ...
            for idx, score in similar_indices:
                if idx < len(self.search_index.vector_index.chunk_metadata):
                    ...
                    result = SearchResult(
                        ...
                        japanese_enhancement=chunk_metadata.get('japanese_enhancement', {})
                    )
        ...
        for chunk_idx, score in sorted_chunks:
            if chunk_idx < len(semantic_index.text_segments):
                result = SearchResult(
                    ...
                    metadata={'chunk_index': chunk_idx, 'matched_keywords': all_keywords}
                )
                results.append(result)
    ...
    async def _execute_faceted_search(
        ...
        if 'category' in facet_conditions:
            ...
            if category in facet_index.categorical_facets:
                for facet_value, chunk_indices in facet_index.categorical_facets[category].items():
        ...
        if 'amount_range' in facet_conditions:
            ...
            if amount_range in facet_index.numerical_facets.get('amount_range', {}):
                matching_chunks.update(facet_index.numerical_facets['amount_range'][amount_range])
        ...
        for chunk_idx in list(matching_chunks)[:k]:
            result = SearchResult(
                ...
                metadata={'chunk_index': chunk_idx, 'facet_conditions': facet_conditions}
            )
            results.append(result)
    ...
    def _rank_fusion(
        ...
        for i, result in enumerate(vector_results):
            ...
            weighted_score = result.relevance_score * hybrid_config.vector_weight * rank_score
        ...
        for i, result in enumerate(semantic_results):
            ...
            rank_score = 1.0 / (i + 1)
            weighted_score = result.relevance_score * hybrid_config.semantic_weight * rank_score
        ...
        for i, result in enumerate(faceted_results):
            ...
            rank_score = 1.0 / (i + 1)
            weighted_score = result.relevance_score * hybrid_config.facet_weight * rank_score
...
class IntelligentQueryProcessor:
    ...
    async def process_query(
        ...
        # Step 3: 検索実行
        search_results = await self._execute_search(query_analysis, query_embedding, options)
        
        # Step 4: 結果後処理
        processed_results = await self._post_process_results(search_results, query_analysis)
        ...
        logger.info(f"Query processed in {execution_time:.2f}ms, {len(processed_results)} results")
    ...
    async def _analyze_query(self, query: str) -> QueryAnalysis:
        ...
        # クエリタイプ決定
        query_type = 'business' if japanese_features.get('has_business_terms') else 'general'
    ...
    async def _vectorize_query(
        ...
        try:
            ...
            # ベクトル生成
            processing_result = await self.vector_processor.process_chunks([query_chunk])
    ...
    async def _execute_search(
        ...
        # 検索意図に基づく検索実行
        if query_analysis.search_intent == 'similarity_search' and query_embedding is not None:
        ...
        elif query_analysis.search_intent in ['faceted_search', 'temporal_search', 'numerical_search']:
    ...
    async def _post_process_results(
        ...
        for result in search_results:
            ...
            if query_analysis.japanese_features.get('has_business_terms', False):
                business_features = result.japanese_enhancement.get('business_features', {})
    ...
    def get_search_suggestions(self, partial_query: str) -> List[str]:
        ...
        if self.search_index.semantic_index:
            ...
            for keyword in keyword_index.keys():
                if keyword.startswith(partial_query) and len(keyword) > len(partial_query):
                    suggestions.append(keyword)
        
        # ビジネス用語サジェスト
        business_terms = [
            '売上高', '純利益', '営業利益', '株式会社', '従業員数', 
            '年度', '四半期', 'ROI', 'KPI', 'EBITDA'
        ]
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'supported_search_methods': [
                'vector_similarity', 'semantic_search', 'faceted_search', 'hybrid_search'
            ]
        }