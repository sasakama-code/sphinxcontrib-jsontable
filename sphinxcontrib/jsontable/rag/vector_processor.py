...
class BusinessTermEnhancer:
    ...
    def __init__(self):
        ...
        self.business_categories = {
            ...
            'financial': {
                'patterns': [r'\d+(?:,\d{3})*円', r'\d+億円', r'\d+万円', r'売上', r'利益', r'収益'],
                'context_marker': '[財務]',
                'boost_factor': 1.4
            },
            'temporal': {
...
class PLaMoVectorProcessor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"PLaMoVectorProcessor initialized with config: {self.model_config}")
    ...
    async def process_chunks(self, chunks: List[SemanticChunk]) -> VectorProcessingResult:
        """チャンクのベクトル処理（非同期）"""
        ...
        logger.info(f"Vector processing completed: {len(vector_chunks)} vectors generated in {processing_time:.2f}s")
        
        return result
    
    async def _enhance_chunks(self, chunks: List[SemanticChunk]) -> List[Dict[str, Any]]:
        """チャンク前処理・文脈強化"""
        ...
        for chunk in chunks:
            ...
            enhanced_data['business_features'] = enhancer.extract_business_features(text)
    ...
    async def _generate_embeddings_batch(self, enhanced_chunks: List[Dict[str, Any]]) -> List[VectorChunk]:
        """バッチ処理によるベクトル生成"""
    ...
    async def _process_batch(self, batch: List[Dict[str, Any]]) -> List[VectorChunk]:
        ...
        for i, (enhanced_data, embedding) in enumerate(zip(batch, embeddings)):
            ...
            japanese_enhancement = {
                ...
                'boost_score': enhanced_data['business_features'].get('boost_score', 1.0)
            }
    ...
    async def _fallback_processing(self, batch: List[Dict[str, Any]]) -> List[VectorChunk]:
        """フォールバック処理"""
    ...
    def _update_processing_stats(self, total: int, successful: int, processing_time: float):
        """処理統計更新"""
        ...
        if total_processed > 0:
            self.processing_stats['average_processing_time'] = (
                (current_avg * (total_processed - total) + processing_time) / total_processed
            )
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        if stats['total_processed'] > 0:
            stats['success_rate'] = stats['successful_embeddings'] / stats['total_processed']
            stats['failure_rate'] = stats['failed_embeddings'] / stats['total_processed']
        else:
