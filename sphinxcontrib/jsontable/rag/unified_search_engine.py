...
class UnifiedSearchEngine:
    ...
    async def index_data(
        ...
        vector_result = await self.vector_processor.process_chunks(
            semantic_chunks, 
            lambda p: progress_callback(0.1 + p * 0.4, "Processing vectors...") if progress_callback else None
        )
        ...
        self._semantic_index = self.semantic_generator.generate(vector_result.vector_chunks)
        
        # Build vector index  
        self._vector_index = self.vector_generator.generate(vector_result.vector_chunks)
        
        # Build faceted index
        self._faceted_index = self.faceted_generator.generate(vector_result.vector_chunks)
        ...
        self.metrics.active_indices = [
            "semantic", "vector", "faceted", "hybrid"
        ]
    ...
    async def search(
        ...
        **kwargs
        ...
        if search_query.query_type == "hybrid":
            ...
            logger.warning(f"Unknown query type: {search_query.query_type}, using hybrid")
            results = await self._hybrid_search(search_query)
        ...
        logger.info(f"Search completed in {query_time:.3f}s, returned {len(results)} results")
    ...
    async def _hybrid_search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        vector_task = self._vector_search(SearchQuery(
            query_text=query.query_text,
            query_type="vector",
            max_results=query.max_results * 2
        ))
        
        semantic_task = self._semantic_search(SearchQuery(
            query_text=query.query_text,
            query_type="semantic", 
            max_results=query.max_results * 2
        ))
        
        faceted_task = self._faceted_search(SearchQuery(
            query_text=query.query_text,
            query_type="faceted",
            filters=query.filters
        )) if query.filters else asyncio.create_task(self._empty_results())
        ...
        fused_scores = self.hybrid_generator.fuse_search_results(
            vector_tuples, semantic_tuples, faceted_ids, 
            self._hybrid_index, query.max_results
        )
        ...
        for chunk_id, score in fused_scores[:query.max_results]:
            if score >= query.min_score:
                result = SearchResult(
                    ...
                    explanation={
                        "vector_score": self._get_vector_score(chunk_id, vector_results),
                        "semantic_score": self._get_semantic_score(chunk_id, semantic_results),
                        "faceted_match": chunk_id in [r.chunk_id for r in faceted_results],
        ...
        for chunk_id, score in matches:
            if score >= query.min_score:
                result = SearchResult(
                    ...
                    metadata=self._get_chunk_metadata(chunk_id),
                    highlighting=self._generate_highlighting(chunk_id, processed_query.keywords)
                )
                results.append(result)
    ...
    async def _vector_search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        if query_embedding is None:
            logger.warning("Failed to generate query embedding, falling back to semantic search")
    ...
    async def _generate_query_embedding(self, query_text: str) -> Optional[np.ndarray]:
    ...
    def _fuse_search_scores(
        ...
        all_chunk_ids = set(vector_scores.keys()) | set(semantic_scores.keys()) | faceted_ids
    ...
    def _post_process_results(
        ...
        filtered_results = [
            ...
            if r.relevance_score >= query.min_score
        ]
    ...
    def is_ready(self) -> bool:
        ...
        return (
            self._semantic_index is not None and
            self._vector_index is not None and
            self._hybrid_index is not None and
            self._faceted_index is not None
        )
