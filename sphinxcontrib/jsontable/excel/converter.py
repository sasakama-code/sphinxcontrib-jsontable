...
class ExcelRAGConverter:
    ...
    Example:
        ...
        ...     excel_file="sales_data.xlsx", rag_purpose="sales-analysis"
        ... )
    ...
    def convert_excel_to_rag(
        ...
        try:
            ...
            result = {
                ...
                "conversion_summary": {
                    ...
                    "entities_detected": len(
                        conversion_result.get("entities", {}).get("items", [])
                    ),
            ...
            logger.info(
                f"Excel-RAG conversion completed successfully. Quality: {quality_score:.2f}"
            )
            return result
    ...
    def process_excel_file(
        self, excel_path: str, config: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        rag_metadata = self.rag_extractor.extract(
            json_data, options={"purpose": config["rag_purpose"], "source": "excel"}
        )
    ...
    def query_excel_data(
        self, excel_file: str | Path, question: str, rag_system: str = "default"
    ) -> str:
    ...
    def _validate_excel_file(self, excel_path: Path) -> None:
        ...
        if file_size_mb > self.default_config["max_file_size_mb"]:
            raise ValueError(
                f"File too large: {file_size_mb:.1f}MB > {self.default_config['max_file_size_mb']}MB"
            )
    ...
    def _generate_rag_metadata(
        ...
        advanced_metadata = {
            "statistics": {
                ...
                "field_count": len(conversion_result.get("json_data", [{}])[0].keys())
    ...
    def _calculate_overall_quality(
        self, conversion_result: dict[str, Any], rag_metadata: dict[str, Any]
    ) -> float:
        ...
        if conversion_result.get("json_data"):
            data_completeness = len(
                [x for x in conversion_result["json_data"] if x]
            ) / max(len(conversion_result["json_data"]), 1)
...
def convert_excel_to_rag(
    excel_file: str | Path, rag_purpose: str, **kwargs
) -> dict[str, Any]:
    """Convenience function for quick Excel-to-RAG conversion.
...
def query_excel_data(excel_file: str | Path, question: str) -> str:
