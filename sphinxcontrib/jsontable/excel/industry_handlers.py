...
class RetailHandler(IndustryHandlerBase):
    ...
    def _extract_product_categories(self, df: pd.DataFrame) -> list[str]:
        ...
        for col in df.columns:
            ...
            if any(
                keyword in col_str
                for keyword in ["カテゴリ", "category", "分類", "商品群"]
            ):
    ...
    def _extract_sales_channels(self, df: pd.DataFrame) -> list[str]:
        ...
        for col in df.columns:
            ...
            if any(
                keyword in col_str
                for keyword in ["チャネル", "channel", "店舗", "オンライン"]
            ):
                with contextlib.suppress(Exception):
    ...
    def _extract_geographic_info(self, df: pd.DataFrame) -> dict[str, list[str]]:
        ...
        geo_info: dict[str, list[str]] = {"regions": [], "stores": [], "cities": []}
    ...
    def _standardize_monetary_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            col_str = str(col).lower()
            if any(
                keyword in col_str for keyword in ["金額", "価格", "売上", "revenue"]
            ):
                try:
                    ...
                    logger.warning(
                        f"Could not standardize monetary values in {col}: {e}"
                    )
        return df
    ...
    def _enrich_sales_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        price_cols = [
            col
            for col in df.columns
            if any(keyword in str(col).lower() for keyword in ["価格", "単価", "price"])
        ]
        qty_cols = [
            col
            for col in df.columns
            if any(
                keyword in str(col).lower() for keyword in ["数量", "quantity", "個数"]
            )
        ]
...
class FinancialHandler(IndustryHandlerBase):
    ...
    def _extract_risk_factors(self, df: pd.DataFrame) -> list[str]:
        ...
        for col in df.columns:
            col_str = str(col).lower()
            if any(
                keyword in col_str for keyword in ["リスク", "risk", "損失", "loss"]
            ):
    ...
    def _standardize_financial_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if any(
                keyword in col_str
                for keyword in ["金額", "価格", "価値", "value", "amount"]
            ):
                try:
                    ...
                    df[col] = (
                        df[col]
                        .astype(str)
                        .str.replace("[円¥,$,%万億兆]", "", regex=True)
                    )
                    df[col] = pd.to_numeric(df[col], errors="coerce")
                except Exception as e:
                    logger.warning(
                        f"Could not standardize financial values in {col}: {e}"
                    )
        return df
