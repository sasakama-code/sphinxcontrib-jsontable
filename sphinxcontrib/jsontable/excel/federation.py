...
class ExcelRAGFederation:
    ...
    Example:
        ...
        >>> federation.add_department("sales", "営業部", [
        ...     {"file": "sales_report.xlsx", "purpose": "sales-analysis"}
        ... ])
        >>> federation.add_department("finance", "財務部", [
    ...
    def add_department(
        ...
        dept_config = DepartmentConfig(
            ...
            access_level=config.get("access_level", "standard") if config else "standard",
        ...
        logger.info(f"Department added successfully: {department_name} ({len(excel_sources)} files)")
    ...
    def add_cross_department_relationship(
        ...
        logger.info(f"Cross-department relationship added: {source_dept} -> {target_dept}")
    ...
    def generate_executive_report(
        ...
        executive_summary = self._create_executive_summary(all_dept_data, overall_metrics)
    ...
    def get_federation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_sync": max(dept.last_updated for dept in self.departments.values()) if self.departments else None,
    ...
    def _process_department_files(
        ...
        for excel_source in dept_config.excel_sources:
            try:
                ...
                result = self.excel_converter.convert_excel_to_rag(
                    ...
                    rag_purpose=excel_source.get("purpose", f"{dept_config.department_id}-analysis"),
                    config={
                        **config,
                ...
                processed_files.append({
                    ...
                    "records_processed": result["conversion_summary"]["records_processed"]
                })
                ...
                if "schema" in metadata:
                    for field in metadata["schema"].keys():
        ...
        return {
            ...
            "overall_quality": sum(overall_quality_scores) / len(overall_quality_scores) if overall_quality_scores else 0,
            "cross_reference_keys": list(set(cross_reference_keys)),
            "available_analysis": available_analysis[:10]  # Limit to top 10
        }
    
    def _update_federation_index(self, department_id: str, dept_results: Dict[str, Any]) -> None:
    ...
    def _generate_persona_report(
        ...
        focus_areas = persona_focus.get(persona, ["総合分析", "パフォーマンス", "改善機会"])
    ...
    def _generate_executive_recommendations(self, all_dept_data: Dict[str, Any]) -> List[str]:
    ...
    def _query_cross_department_data(self, query: str, departments: List[str]) -> Dict[str, Any]:
