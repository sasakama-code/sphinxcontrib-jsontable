...
class AutoSphinxIntegration:
    ...
    def create_complete_documentation(
        self, conversion_result: dict[str, Any], config: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        logger.info(
            f"Complete Sphinx documentation created: {len(written_files)} files"
        )
        return result
    ...
    def _create_main_document(
        ...
        excel_source = conversion_result.get("metadata", {}).get(
            "source_file", "Unknown"
        )
        ...
        quality_report = self._format_quality_report(
        ...
        try:
            main_document = template.format(
                ...
                keywords=", ".join(
                    conversion_result.get("entities", {}).get("types", [])
                ),
...
"""
    ...
    def _create_index_document(
        self, conversion_result: dict[str, Any], main_doc: str, sub_docs: list[str]
    ) -> str:
    ...
    def _update_sphinx_config(
        ...
        sphinx_config = {
            "extensions": ["sphinxcontrib.jsontable", "sphinxcontrib.jsontable.excel"],
            ...
            "html_theme_options": {"excel_integration": True, "rag_features": True},
        }
    ...
    def _create_file_structure(
        ...
        structure = {
            "docs/": {
                ...
                "api/": {"excel_integration.rst": "API documentation"},
            }
        }
...
""",
    ...
    def _infer_title(
        self, conversion_result: dict[str, Any], config: dict[str, Any] | None = None
    ) -> str:
        ...
        rag_purpose = (
            config.get("rag_purpose", "data-analysis") if config else "data-analysis"
        )
        ...
        return title_map.get(
            rag_purpose, f"{rag_purpose.replace('-', ' ').title()} データ分析"
        )
...
"""
        ...
        for _i, col in enumerate(columns[:10]):  # Show first 10 columns
...
"""
    ...
    def _create_chunk_document(
        ...
        total_chunks = (
            len(conversion_result.get("json_data", [])) // records_per_chunk + 1
        )
        chunk_navigation = ""
