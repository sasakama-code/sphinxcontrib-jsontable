...
class ExcelRAGMonitor:
    ...
    Example:
        ...
        >>> monitor.watch_file(
        ...     "sales_report.xlsx", on_change=lambda: print("Sales data updated!")
        ... )
    ...
    def __init__(
        self,
        excel_converter: ExcelRAGConverter | None = None,
        federation: ExcelRAGFederation | None = None,
        ...
        # Threading
        self.batch_processor_thread: threading.Thread | None = None
    ...
    def watch_directory(
        ...
        # Scan existing files
        existing_files = self._scan_directory_for_excel_files(
            dir_path, file_pattern, recursive
        )
        ...
        logger.info(
            f"Watching directory: {directory_path} ({len(existing_files)} Excel files found)"
        )
    ...
    def start_monitoring(self) -> None:
        ...
        logger.info(
            f"Excel monitoring started for {len(self.watched_files)} files and {len(self.watched_directories)} directories"
        )
    ...
    def force_update_all(self) -> dict[str, Any]:
        ...
        for file_path, _watch_config in self.watched_files.items():
            try:
    ...
    def _handle_file_change(
        self, file_path: str, event_type: str, timestamp: datetime
    ) -> None:
    ...
    def _process_change_event(self, event: ExcelFileChangeEvent) -> None:
        ...
        try:
            ...
            result = self._process_file_update(
                event.file_path, event.event_type, event.timestamp
            )
    ...
    def _process_file_update(
        ...
        if quality_score < self.update_policy.quality_threshold:
            raise ValueError(
                f"Quality score {quality_score} below threshold {self.update_policy.quality_threshold}"
            )
        ...
        return {
            ...
            "records_processed": result.get("conversion_summary", {}).get(
                "records_processed", 0
            ),
    ...
    def _scan_directory_for_excel_files(
        self, directory: Path, pattern: str, recursive: bool
    ) -> list[Path]:
    ...
    def _process_batch(self, changes: list[ExcelFileChangeEvent]) -> None:
        ...
        logger.info(
            f"Batch processing completed: {processed_count}/{batch_size} successful"
        )
    ...
    def _create_backup(self, file_path: str) -> str:
        ...
        backup_path = (
            file_obj.parent / f"{file_obj.stem}_backup_{timestamp}{file_obj.suffix}"
        )
...
def monitor_department_files(
    department_files: dict[str, list[str]], federation: ExcelRAGFederation | None = None
) -> ExcelRAGMonitor:
