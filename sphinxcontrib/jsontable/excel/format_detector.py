...
class ExcelFormatHandler:
    ...
    def get_handler(self, format_type: str) -> 'BaseFormatHandler':
        ...
        return self.format_handlers.get(format_type, self.format_handlers['standard_table'])
...
class AdvancedExcelConverter:
    ...
    def analyze_structure(self, excel_file: str) -> Dict[str, Any]:
        ...
        structure["overall_format"] = self._determine_overall_format(structure["sheets"])
        structure["complexity_score"] = self._calculate_complexity_score(structure["sheets"])
    ...
    def convert_with_intelligence(self, excel_file: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for sheet_name, sheet_data in excel_structure["sheets"].items():
            if "sample_data" in sheet_data:
                ...
                if sample_data:
                    ...
                    for col_idx, col_data in enumerate(sample_data[0] if sample_data else []):
    ...
    def extract_entities(
        ...
        entities["types"] = list(set(item["type"] for item in entities["items"]))
    ...
    def _analyze_sheet_structure(self, sheet: Worksheet, sheet_name: str) -> Dict[str, Any]:
    ...
    def _has_financial_indicators(self, sheet: Worksheet) -> bool:
        ...
        financial_keywords = [
            '売上', '売上高', '売上総利益', '営業利益', '当期純利益',
            '資産', '負債', '純資産', '流動資産', '固定資産',
            'Revenue', 'Sales', 'Profit', 'Assets', 'Liabilities'
        ]
    ...
    def _has_crosstab_structure(self, sheet: Worksheet) -> bool:
        ...
        if sample_data:
            for col_idx in range(len(sample_data[0])):
                col_data = [row[col_idx] for row in sample_data[1:] if col_idx < len(row)]
                numeric_count = sum(1 for cell in col_data if isinstance(cell, (int, float)))
    ...
    def _calculate_complexity_score(self, sheets: Dict[str, Any]) -> float:
        ...
        for sheet_data in sheets.values():
            ...
            size_score = min(1.0, (dimensions.get("max_row", 0) * dimensions.get("max_column", 0)) / 10000)
    ...
    def _generate_comprehensive_metadata(
        ...
        return {
            ...
            "data_quality": {
                "completeness": self._calculate_completeness(conversion_result.get("data", [])),
    ...
    def _calculate_consistency(self, data: List[Dict[str, Any]]) -> float:
        ...
        consistent_records = sum(1 for record in data if set(record.keys()) == first_keys)
    ...
    def _infer_column_type(self, sample_data: List[List[Any]], col_idx: int) -> str:
        ...
        col_values = [row[col_idx] for row in sample_data[1:] if col_idx < len(row) and row[col_idx] is not None]
    ...
    def _convert_sheet_to_json(
        ...
        headers = [str(cell) if cell is not None else f"column_{i}" for i, cell in enumerate(sample_data[0])]
