...
class DataFrameMemoryPool:
    ...
    def acquire_dataframe(self, shape: Tuple[int, int], dtypes: Union[Dict[str, str], pd.Series]) -> PooledDataFrame:
        """DataFrameの取得
    ...
    def _acquire_dataframe_unsafe(self, shape: Tuple[int, int], dtypes: Union[Dict[str, str], pd.Series], start_time: Optional[float]) -> PooledDataFrame:
        """DataFrame取得（ロックなし版）"""
        # メモリ制限チェック
        if self.enable_memory_monitoring:
            current_memory_mb = self.get_current_memory_usage() / 1024 / 1024
            if current_memory_mb > self.max_memory_mb:
                raise MemoryError(f"Memory limit exceeded: {current_memory_mb:.1f}MB > {self.max_memory_mb}MB")
    ...
    def _release_dataframe_unsafe(self, pooled_df: PooledDataFrame, start_time: Optional[float]):
        """DataFrame返却（ロックなし版）"""
        # プールサイズ制限チェック
    ...
    def _normalize_dtypes(self, dtypes: Union[Dict[str, str], pd.Series]) -> Dict[str, str]:
        """dtypes正規化"""
    ...
    def _find_matching_dataframe(self, shape: Tuple[int, int], dtypes: Dict[str, str]) -> Optional[str]:
        """適合するDataFrame検索"""
    ...
    def _is_compatible(self, shape: Tuple[int, int], dtypes: Dict[str, str], pooled_df: PooledDataFrame) -> bool:
        """DataFrameの互換性チェック"""
    ...
    def _update_performance_stats(self):
        ...
        if self._hit_count > 0 and self._pool:
            avg_memory_per_df = sum(
                df.memory_size_bytes for df in self._pool.values() if df.memory_size_bytes > 0
            )
            if avg_memory_per_df > 0:
                avg_memory_per_df = avg_memory_per_df / len([
                    df for df in self._pool.values() if df.memory_size_bytes > 0
                ])
                estimated_memory_saved = (self._hit_count * avg_memory_per_df) / 1024 / 1024
    ...
    def get_efficiency_statistics(self) -> Dict[str, float]:
        """効率統計取得"""
        return {
            'gc_reduction_ratio': min(1.0, self._stats['gc_reduction_count'] / max(1, self._miss_count + self._hit_count)),
