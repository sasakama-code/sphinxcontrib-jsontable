...
class IDataConverter(ABC):
    ...
    @abstractmethod
    def convert_dataframe_to_json(self, df: pd.DataFrame, has_header: bool = True) -> ConversionResult:
    ...
    @abstractmethod
    def normalize_headers(self, headers: List[str], japanese_support: bool = True) -> List[str]:
...
class DataConverter(IDataConverter):
    ...
    def __init__(self, 
        ...
        self.header_keywords = header_keywords or [
            "名前", "name", "氏名", "項目", "item", "title", "タイトル",
            "id", "識別子", "番号", "no", "number", "date", "日付", "時間", "time"
        ]
    ...
    def detect_header(self, df: pd.DataFrame) -> HeaderDetectionResult:
        ...
        try:
            ...
            analysis = {
                ...
                "factors_analyzed": ["string_ratio", "numeric_ratio", "keyword_presence"]
            }
    ...
    def normalize_headers(self, headers: List[str], japanese_support: bool = True) -> List[str]:
        ...
        try:
            ...
            for i, header in enumerate(headers):
                ...
                if not normalized_header or normalized_header.lower() in ['nan', 'none', 'null']:
                    normalized_header = f"Column_{i+1}"
                
                # Apply Japanese-specific normalization if enabled
                if japanese_support:
                    normalized_header = self._normalize_japanese_header(normalized_header)
    ...
    def _calculate_header_confidence(self, string_ratio: float, 
                                   numeric_ratio: float, keyword_match: bool) -> float:
...
class MockDataConverter(IDataConverter):
    ...
    def __init__(self, 
        ...
        self.error_to_raise = error_to_raise or DataConversionError("mock_operation", message="Mock error")
