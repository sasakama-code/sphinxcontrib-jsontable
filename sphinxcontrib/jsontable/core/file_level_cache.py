...
class FileLevelCache:
    ...
    def generate_cache_key(self, file_path: Path, processing_options: Dict[str, Any]) -> CacheKey:
        """キャッシュキー生成
        ...
        try:
            ...
            return CacheKey(
                file_path=str(file_path),
                unique_key=f"fallback_{time.time()}"
            )
    ...
    def put(self, cache_key: CacheKey, data: Any) -> CacheEntry:
        ...
        with self._lock:
            ...
            if self.config.enable_compression:
                ...
                storage_entry.compressed_size = len(compressed_data) if isinstance(compressed_data, bytes) else data_size
            else:
    ...
    def _evict_lru_items(self):
        """LRU削除実行"""
        while len(self._cache_storage) >= self.config.max_cache_size and self._access_order:
    ...
    def _compress_data(self, data: Any) -> bytes:
        ...
        try:
            ...
            if self.config.compression_algorithm == 'gzip':
                return gzip.compress(pickled_data, compresslevel=self.config.compression_level)
            return pickled_data
        except Exception as e:
    ...
    def _update_hit_ratio(self):
        ...
        if total_requests > 0:
            self._statistics['hit_ratio'] = self._statistics['cache_hits'] / total_requests
    ...
    def _check_memory_cleanup(self):
        ...
        threshold_mb = (self.config.memory_cleanup_threshold / 100.0) * self.config.max_memory_mb
        
        if current_mb > threshold_mb:
            logger.info(f"Memory cleanup triggered: {current_mb:.1f}MB > {threshold_mb:.1f}MB")
    ...
    def get_performance_statistics(self) -> Dict[str, Any]:
        ...
        if self._statistics['cache_hits'] > 0:
            ...
            # 平均レスポンス時間更新
            self._performance_stats['cache_hit_time_avg'] = 0.001 * (1 - hit_ratio * 0.5)
            self._performance_stats['cache_miss_time_avg'] = 0.100 + (1 - hit_ratio) * 0.05
    ...
    def _calculate_cache_efficiency(self) -> float:
        ...
        try:
            ...
            compression_effectiveness = min(1.0, self._compression_stats['compression_ratio'] / 3.0)
            
            # 総合効率スコア（0.0-1.0）
            efficiency = (hit_ratio * 0.5 + memory_efficiency * 0.3 + compression_effectiveness * 0.2)
