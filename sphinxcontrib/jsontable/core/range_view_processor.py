...
class RangeViewData:
    ...
    def __post_init__(self):
        ...
        if not self.data_source:
            self.data_source = self.view_id.split('_')[0] if '_' in self.view_id else 'unknown'
...
class RangeViewProcessor:
    ...
    def get_range_view(self, file_path: Union[str, Path], start_row: int, end_row: int) -> RangeViewData:
        """範囲ビューデータ取得
        ...
        if start_row >= end_row:
            raise ValueError(f"Invalid range: start_row ({start_row}) must be less than end_row ({end_row})")
        ...
        try:
            ...
            # 範囲妥当性確認
            if start_row >= len(df):
                raise ValueError(f"start_row ({start_row}) exceeds file length ({len(df)})")
            ...
            # 操作履歴記録
            self._record_operation_history(file_key, start_row, actual_end_row, view_creation_time, memory_delta)
    ...
    def _chunked_dict_conversion(self, df: pd.DataFrame, chunk_size: int = 1000) -> List[Dict[str, Any]]:
    ...
    def _record_operation_history(self, file_key: str, start_row: int, end_row: int, 
                                  creation_time: float, memory_delta: int):
        """操作履歴記録
    ...
    def get_range_view_with_fallback(self, file_path: Union[str, Path], start_row: int, end_row: int) -> RangeViewData:
        """フォールバック付き範囲ビュー取得
    ...
    def _perform_view_operation(self, file_path: Union[str, Path], start_row: int, end_row: int) -> RangeViewData:
        """ビュー操作実行（テストモック用）"""
        return self.get_range_view(file_path, start_row, end_row)

    def _fallback_traditional_method(self, file_path: Union[str, Path], start_row: int, end_row: int) -> RangeViewData:
        """従来方式フォールバック"""
        file_path = Path(file_path)
        df = pd.read_excel(file_path)
    ...
    def read_chunks_with_views(self, file_path: Union[str, Path]) -> Iterator[RangeViewData]:
        """ビュー操作付きチャンク読み込み
    ...
    def _update_view_statistics(self, creation_time: float, data_size: int):
        ...
        if self.enable_view_optimization:
            ...
            if len(self._operation_history) > 5:
                ...
                avg_memory_delta = sum(op['memory_delta'] for op in recent_operations) / len(recent_operations)
                # 効率性 = 低メモリ使用量ほど高効率
                efficiency = max(0.5, 1.0 - (avg_memory_delta / (50 * 1024 * 1024)))  # 50MB基準
        ...
        if self.enable_data_validation:
            error_rate = self._error_count / max(1, self._view_stats['view_operations_count'])
        ...
        if self.enable_view_caching:
            ...
            if total_operations > 0:
                cache_eligible_ops = max(0, total_operations - len(self._loaded_dataframes))
                self._view_stats['cache_hit_ratio'] = cache_eligible_ops / total_operations
        
        # 並行操作統計（実際の並行性考慮）
        if self.enable_concurrent_views:
            self._view_stats['concurrent_operations_count'] = self._view_stats['view_operations_count']
            # スレッドセーフティ違反チェック
    ...
    def get_concurrent_statistics(self) -> Dict[str, int]:
        """並行操作統計取得"""
        return {
            'concurrent_operations_count': self._view_stats['concurrent_operations_count'],
    ...
    def get_integration_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'streaming_efficiency_improvement': self._view_stats['streaming_efficiency_improvement']
        }
