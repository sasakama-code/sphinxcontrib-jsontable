...
class UnifiedDataConversionEngine:
    ...
    def __init__(
        ...
        self.performance_analyzer = (
            performance_analyzer or ConversionPerformanceAnalyzer()
        )
        ...
        self.header_processor = OptimizedHeaderProcessor(
            excel_reader=mock_excel_reader, data_converter=mock_data_converter
        )
    ...
    def process_file_with_deduplication(
        ...
        try:
            ...
            self.deduplication_stats["intermediate_objects_reduced"] = (
                INTERMEDIATE_OBJECTS_REDUCTION
            )
            self.deduplication_stats["pipeline_stages_optimized"] = (
                PIPELINE_STAGE_REDUCTION
            )
    ...
    def process_with_caching(
        ...
        # ファイル変更検出
        file_change_detected = self.cache_manager.check_file_change(
            file_path, cache_key
        )
        ...
        self.cache_manager.store_result(
            cache_key, {"data": result["data"], "headers": result["headers"]}, file_path
        )
    ...
    def _generate_cache_key(self, file_path: Path, options: Dict[str, Any]) -> str:
        ...
        key_components = [str(file_path), str(options)]
...
class SinglePassProcessor:
    ...
    def execute_single_pass_conversion(
        self, file_path: Path, conversion_options: Dict[str, Any]
    ) -> UnifiedConversionResult:
        """単一パス変換実行"""
        start_time = time.perf_counter()
...
class MemoryOptimizedConverter:
    ...
    def __init__(
        ...
        self.enable_garbage_collection_optimization = (
...
class ConversionCacheManager:
    ...
    def store_result(
        self, cache_key: str, result: Dict[str, Any], file_path: Optional[Path] = None
    ) -> None:
        """結果キャッシュ保存"""
        self.cache_storage[cache_key] = result

        metadata = {"stored_at": time.time(), "access_count": 0}
