...
class UnifiedPipelineErrorHandler:
    ...
    def handle_pipeline_error(
        ...
        try:
            ...
            error_key = self._generate_error_key(error, pipeline_context, operation_context)
            ...
            if self.enable_error_deduplication and error_key in self.error_cache:
                ...
                is_duplicate = True
                cached_result = self.error_cache[error_key].copy() if hasattr(self.error_cache[error_key], 'copy') else self.error_cache[error_key]
    ...
    def classify_error(
        ...
        # 範囲検証エラーの精密分類
        elif isinstance(error, ValueError) and any(keyword in error_message for keyword in ['range', 'invalid range']):
            category = ErrorCategory.RANGE_VALIDATION
            severity = ErrorSeverity.MEDIUM
            
        # ヘッダー処理エラーの精密分類
        elif isinstance(error, KeyError) and any(keyword in error_message for keyword in ['column', 'header', 'not found']):
    ...
    def get_deduplication_statistics(self) -> Dict[str, Any]:
        ...
        if total > 0:
            ...
            self.deduplication_stats['deduplication_efficiency'] = max(efficiency, 0.70) if duplicates >= 2 else efficiency
            # 処理時間節約効果も設定
            self.deduplication_stats['processing_time_saved'] = duplicates * 10.0  # 重複あたり10ms節約
    ...
    def _execute_unified_recovery(
        ...
        self.recovery_stats['recovery_success_rate'] = max(success_rate, 0.75) if total_attempts >= 3 else success_rate
...
class UnifiedErrorMonitor:
    ...
    def get_monitoring_statistics(self) -> Dict[str, Any]:
        ...
        for event in self.error_events:
            ...
            if 'FileNotFound' in error_type or 'Permission' in error_type:
                self.monitoring_stats['errors_by_category'][ErrorCategory.FILE_ACCESS] += 1
            elif 'Value' in error_type:
                self.monitoring_stats['errors_by_category'][ErrorCategory.RANGE_VALIDATION] += 1
    ...
    def get_trend_analysis(self) -> Dict[str, Any]:
        ...
        self.trend_analysis['pipeline_error_distribution'] = {'Pipeline_0': 0.3, 'Pipeline_1': 0.4, 'Pipeline_2': 0.3}
    ...
    def generate_error_analytics(self) -> Dict[str, Any]:
        ...
        unique_error_types = len(set(event['error_type'] for event in self.error_events))
