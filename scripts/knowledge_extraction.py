...
class KPTAnalyzer:
    ...
    def parse_kpt_file(self, kpt_file_path: str) -> dict[str, Any]:
        ...
        with open(kpt_file_path, encoding="utf-8") as f:
            content = f.read()
        ...
        # 実装詳細抽出
        implementation_details = re.findall(
            self.kpt_patterns["implementation_details"], content, re.DOTALL
        )
        analysis_result["implementation_details"] = implementation_details

        # 定量的結果抽出
        quantitative_results = re.findall(
            self.kpt_patterns["quantitative_results"], content
        )
            ...
            for item in keep_items:
                if self._meets_extraction_criteria(item):
                    candidate = {
                        ...
                        "replication_potential": self._assess_replication_potential(
                            item
                        ),
    ...
    def _parse_try_items(self, try_content: str) -> list[dict[str, Any]]:
        ...
        for i in range(1, len(items), 2):
            if i + 1 < len(items):
                ...
                # 期待効果抽出
                impact = re.findall(
                    r"\*\*期待効果\*\*:(.*?)(?=\*\*|$)", content, re.DOTALL
                )
                approach = re.findall(
                    r"\*\*アプローチ\*\*:(.*?)(?=\*\*|$)", content, re.DOTALL
                )
...
"""
    ...
    def generate_knowledge_document(
        self, candidate: dict[str, Any], kpt_analysis: dict[str, Any]
    ) -> str:
        """ナレッジ文書生成"""
    ...
    def _extract_implementation_details(self, candidate: dict[str, Any]) -> str:
        ...
        if code_blocks:
            ...
            # 実装に関する記述を検索
            impl_patterns = [r"実装[^。]*。", r"手法[^。]*。", r"アプローチ[^。]*。"]
            ...
            return (
                ...
                if implementations
                else "具体的な実装手順は元のKPT分析を参照してください。"
            )
...
class KnowledgeValidator:
    ...
    def _evaluate_criterion(self, content: str, criterion: str) -> float:
        ...
        evaluation_map = {
            "has_empirical_evidence": lambda c: 1.0
            if re.search(r"実証|実績|データ|結果|成果", c)
            else 0.0,
            "includes_concrete_examples": lambda c: 1.0
            if re.search(r"```|例:|実装|コード", c)
            else 0.0,
            "defines_success_metrics": lambda c: 1.0
            if re.search(r"指標|メトリクス|測定|[0-9]+%", c)
            else 0.0,
            "specifies_constraints": lambda c: 1.0
            if re.search(r"前提|条件|制約|要件", c)
            else 0.0,
            "follows_standard_format": lambda c: 1.0
            if re.search(r"# 🎯.*\*\*抽出元\*\*.*## 📋 概要", c, re.DOTALL)
            else 0.0,
            "has_searchable_keywords": lambda c: 1.0
            if re.search(r"🎯|📋|🔧|📊", c)
            else 0.0,
            "includes_code_examples": lambda c: min(
                1.0, len(re.findall(r"```", c)) / 4
            ),
            "proper_categorization": lambda c: 1.0
            if re.search(r"technical_|development_|business_|project_|lessons_", c)
            else 0.0,
            "replication_feasibility": lambda c: 1.0
            if re.search(r"手順|ステップ|実装|方法", c)
            else 0.0,
            "clear_implementation_steps": lambda c: min(
                1.0, len(re.findall(r"\d+\.|###|\*\*手順", c)) / 5
            ),
            "troubleshooting_info": lambda c: 1.0
            if re.search(r"注意|問題|エラー|対応", c)
            else 0.0,
            "version_info": lambda c: 1.0
            if re.search(r"\d{4}-\d{2}-\d{2}|\d{4}年\d{1,2}月", c)
            else 0.0,
            "update_schedule": lambda c: 1.0
            if re.search(r"更新|見直し|レビュー", c)
            else 0.0,
            "related_documents": lambda c: 1.0
            if re.search(r"関連|参照|参考", c)
            else 0.0,
            "review_history": lambda c: 1.0
            if re.search(r"履歴|バージョン|変更", c)
            else 0.0,
        }
    ...
    def _generate_recommendations(
        ...
        for dimension, results in validation_results.items():
            if results["score"] < 0.7:
                low_criteria = [
                    criterion
                    for criterion, score in results["details"].items()
                    if score < 0.5
                ]

                if low_criteria:
                    recommendations.append(
                        f"{dimension}の改善が必要: {', '.join(low_criteria)}"
                    )
...
def main():
    ...
    extract_parser.add_argument(
        "--output-dir", default="knowledge/", help="出力ディレクトリ"
    )
    extract_parser.add_argument(
        "--auto-generate", action="store_true", help="自動文書生成"
    )
    ...
    review_parser.add_argument(
        "--knowledge-file", required=True, help="レビュー対象ファイル"
    )
...
def pre_check_knowledge(args):
    ...
    if not md_files:
        print(
            "📋 ナレッジファイルが見つかりません。新規パターンとして慎重に進行してください。"
        )
        return
...
def extract_task_keywords(task_description: str) -> list[str]:
    ...
    all_keywords = (
        technical_keywords + business_keywords + process_keywords + branch_keywords
    )
...
def generate_task_guidance(
    task_description: str, relevant_knowledge: list[dict[str, Any]]
) -> str:
    """タスクガイダンス生成"""
...
"""
    ...
    for i, knowledge in enumerate(relevant_knowledge[:5], 1):
        ...
        guidance += (
            f"- **マッチキーワード**: {', '.join(knowledge['matching_keywords'])}\n"
        )
...
def extract_knowledge(args):
    ...
    for i, candidate in enumerate(candidates, 1):
        print(
            f"{i}. {candidate['title']} ({candidate['suggested_category']}) - {candidate['security_level']}"
        )
    ...
    if args.auto_generate:
        ...
        for candidate in candidates:
            if candidate["security_level"] in ["internal", "confidential"]:
                # ナレッジ文書生成
                document_content = generator.generate_knowledge_document(
    ...
    if args.knowledge_file:
        ...
        for md_file in md_files:
            ...
            if "error" not in result:
                ...
                print(
                    f"  {md_file.name}: {result['overall_quality_score']:.2f} ({result['approval_status']})"
                )
...
def check_branch_strategy(task_description: str) -> dict[str, Any]:
    ...
    try:
        ...
        branch_check_result = {
            ...
            "recommendation": f"推奨ブランチ: {recommended_branch}"
            if not is_appropriate
            else f"現在のブランチ({current_branch})は適切です",
        }
...
def is_branch_appropriate(current_branch: str, task_description: str) -> bool:
    ...
    if current_branch.startswith("feature/") and recommended_branch.startswith(
        "feature/"
    ):
        # 同一フェーズのブランチなら適切とする
